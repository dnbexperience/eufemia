...
version: 11.8.0
generatedAt: 2026-06-26T12:38:08.295Z
...
Inside `./packages/dnb-eufemia`, you will find the directory `/src/components` or `/src/extensions`. There you can place a new directory with all the necessary subfolders. As a reference, take a look at Component folder section in [Before getting started](/contribute/first-contribution/before-started#component-folder).

Run the Eufemia Portal with `yarn start`. Make sure you follow the [Code guide](/contribute/style-guides/coding) during development.
...
SCSS file names starting with `dnb-` can later be imported as individual packages:

- `./packages/dnb-eufemia/src/components/button/style/dnb-button.scss`

### Style dependencies

In order to test related style dependencies of components, we add style imports in the `deps.scss` file, which is then used in Jest tests to perform a snapshot comparison:
...
Theming file names ending with `-ui` will be packed into the global theming package during release. More details in the [theming section](/uilib/usage/customisation/theming).
...
Running the build command will walk through all parts and tie together the needed pieces in order to generate valid build bundles.
...
Put your translation inside: `./packages/dnb-eufemia/src/shared/locales/nb-NO.js` as well as in the `en-GB.js` file:
...
The function `getTranslation` will, along with the properties, support both `locale` and the HTML `lang` attribute. This way, these properties can be set on a per-component basis and a per-context basis.
...
How you make [spacing](/uilib/layout/space) support available varies from case to case. But you may always allow the developer to pass in the spacing properties to the very root element of your component.
...
You may have a look at the [Documentation guide](/contribute/style-guides/documentation) and existing docs in order to get the right structure.
