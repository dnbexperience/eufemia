...
- [Stat.Info](/uilib/components/stat/info): Renamed `variant="default"` to `variant="plain"`. The old value is deprecated and will be removed in a future version.
...
- New components in the `@dnb/eufemia`: [ToggleButton](/uilib/components/toggle-button) as well as two layout components [FormSet](/uilib/layout/form-set) and [FormRow](/uilib/layout/form-row)
