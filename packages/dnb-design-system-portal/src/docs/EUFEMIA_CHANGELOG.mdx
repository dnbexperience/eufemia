...
- [New major version 11](/uilib/about-the-lib/releases/eufemia/v11/)
...
- All Eufemia styles are now wrapped in a `@layer dnb-eufemia` [CSS cascade layer](https://developer.mozilla.org/en-US/docs/Web/CSS/@layer). Consumer styles without `@layer` automatically override Eufemia styles without needing `!important`. A standalone `layers.scss` file is also available for pre-declaring the layer order.
