...
- [Dropdown](/uilib/components/dropdown): The `preventSelection`, `actionMenu`, and `moreMenu` props have been replaced with a unified `mode` prop. Use `mode="prevent"`, `mode="action-menu"`, or `mode="more-menu"` instead. This is a breaking change in v11.
