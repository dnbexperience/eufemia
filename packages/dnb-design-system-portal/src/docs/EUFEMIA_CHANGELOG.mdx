...
- Standardized `size` prop values across all components: removed `'default'`, replaced `FormLabel`'s `'basis'` with `'small'`, and renamed `Icon`'s `'default'` to `'basis'`. See [v11 migration guide](/uilib/about-the-lib/releases/eufemia/v11-info/#standardized-size-values).
