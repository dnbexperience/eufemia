...
- **Software Bill of Materials (SBOM)** — an SBOM is generated and attached per release. It is delivered separately in [#8812](https://github.com/dnbexperience/eufemia/pull/8812).
...
The library is built once into `build/`, and `validate:package` inspects the exact contents `npm pack` would publish. In `consumer-smoke.yml` the build is then packed once and both the Vite and Next.js consumers install and build that same tarball. At release time, `@semantic-release/npm` is configured with `tarballDir`, so it packs the versioned tarball once and publishes that exact file (also attached to the GitHub release with provenance).

The one remaining gap: `validate:package` and the smoke tests run on the pre-release build (version `0.0.0-development`), whose file contents are identical to the published tarball except for the stamped version — they do not yet run against the exact in-flight release tarball inside the `semantic-release` run. Closing that (a `semantic-release` prepare-time validation of the packed `dist/` tarball) is a tracked follow-up.
