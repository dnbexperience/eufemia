...
Run the [Design Tokens](https://github.com/dnbexperience/eufemia/actions/workflows/tokens-lib.yml) workflow from the GitHub Actions tab.

It reads the Figma variables with the [variables REST API](https://developers.figma.com/docs/rest-api/variables-endpoints/), writes the `*.tokens.json` files listed below, regenerates the CSS files, updates the visual test snapshots and opens a draft pull request on the `tokens/automation-figma-sync` branch.

Review the token values, removals and visual snapshots before marking the pull request as ready.
