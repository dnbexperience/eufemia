...
The workflow needs the repository secrets `FIGMA_TOKEN` (a personal access token with the `file_variables:read` scope) and `FIGMA_TOKENS_FILE` (the key of the "💻 Eufemia - Web" file).
