...
### What does `build:gatsby:mini` do?

When running `build:gatsby:mini`, the Gatsby build will be faster and the output will be smaller. It was used for the CI/CD pipeline to build the package for the Sandbox or documentation preview. The default build now uses Vite (`yarn build`).
