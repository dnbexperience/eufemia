...
Components are written in [TypeScript](https://www.typescriptlang.org/) for type safety and better developer experience.
...
- `jest.config.screenshots.js` configures [jest-image-snapshot](https://github.com/americanexpress/jest-image-snapshot), which is related to the screenshot testing (visual assertion).
...
As of now, some components are written in TypeScript (`.tsx`) and some in JavaScript with type definition files (`.d.ts`).
...
**NB:** After you have synced/updated the files, you will see that some newlines were removed. That's a negative side-effect for which we have no solution at the moment.
...
The portal is handled by [Gatsby](https://www.gatsbyjs.com/), a framework for building pre-rendered SSR websites.
...
And visit e.g. this page: [http://localhost:8000/uilib/components/button/](http://localhost:8000/uilib/components/button/)
