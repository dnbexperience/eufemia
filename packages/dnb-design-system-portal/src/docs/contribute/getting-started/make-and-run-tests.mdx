...
describe.each(selectBrands(['ui', 'sbanken', runOnMain('eiendom')]))(
  'Button for %s',
  (themeName) => {
    setupPageScreenshot({
      themeName,
...
`runOnMain(...)` only includes that theme when the current branch is `main` or starts with `v` followed by a digit (for example `v11` or `v11-fix`).
