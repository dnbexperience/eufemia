...
For only checking valid formatting, you can run `yarn workspace @dnb/eufemia prettier:check`. You may want to run `yarn workspace @dnb/eufemia prettier:write` to formatt all relevant files.

Fix the resulted warnings and error before you commit and merge.

The same commands as above applies to the workspace: `yarn worksapce dnb-design-system-portal ...`.
