---
title: 'Secondary'
contentTitle: 'Secondary Icons'
description: 'The secondary icons will be extended over time to include all the often-used Eufemia icons.'
icon: 'secondary'
order: 3
---

import ListAllIcons from 'dnb-design-system-portal/src/shared/parts/icons/ListAllIcons'

The Secondary Icons are an addition to the [Primary Icons](/icons/primary). They extend the possibility to have more icons that are not primarily used. They do not ship integrated, as the [Primary Icons](/icons/primary) do.

The Secondary Icons can be extended infinitely.

**Technically:** As a consequence, they have to be [imported](/uilib/components/icon) wherever they are used.

## React example usage

```jsx
import { hamburger_medium as HamburgerIcon, bubble } from '@dnb/eufemia/icons'

<HamburgerIcon />/* <-- is not recommended, but possible */
<Icon icon={HamburgerIcon} />
<Button icon={bubble} />
```

## A list of all Secondary Icons

<ListAllIcons variant="secondary" />
