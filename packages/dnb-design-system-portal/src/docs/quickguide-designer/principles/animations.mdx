...
- [Switch](/uilib/components/switch) with its short feedback animation.
- [TextCounter](/uilib/components/fragments/text-counter) with its error message appearance.

### Helpers to use in code

Timing and easing matter.

- Keep direct interaction feedback short. Try around `180ms` for a small movement, and `300ms` to `500ms` for larger transitions or expanding content.
- Avoid delaying feedback after a user action.
- Prefer animating `transform` and `opacity` when possible.
- Use the [HeightAnimation](/uilib/components/height-animation/) component when content with an unknown height expands or collapses.
- Reuse Eufemia's [animation easing variables](/uilib/helpers#animation-easing) in custom CSS. Use `--easing-default` for standard transitions:

```css
transition: height 400ms var(--easing-default);
```

Use `--easing-fast-bounce` for short, direct interactions that benefit from a subtle settling effect. Keep it to small movements, such as a control changing state:
