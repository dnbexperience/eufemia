...
You can download the [latest stable DNB font files as a ZIP package](https://assets.eufemia.dnb.no/fonts/dnb/DNB.zip). Use the matching [versioned Asset CDN URL](/uilib/usage/assets/#stable-and-versioned-urls) when a build must stay tied to one Eufemia release.
