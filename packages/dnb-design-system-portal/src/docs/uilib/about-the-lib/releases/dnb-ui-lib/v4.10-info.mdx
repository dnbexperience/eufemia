...
- [DatePicker](/uilib/components/date-picker) is now using v2 of `date-fns` and with this it's more easy to translate to English (including new properties `submit_button_text` and `cancel_button_text`), as Norwegian (`nb`) is the default `locale`. But also `align_picker` is a nice feature to have.
