...
- Sortable [table](/uilib/components/table) header buttons supports now word-wrap by using `wrap="true"`.
- [Table](/uilib/components/table) headers supports now `small` font-size (`.dnb-table--small`) as well as `right` and `center` aligned headers.
