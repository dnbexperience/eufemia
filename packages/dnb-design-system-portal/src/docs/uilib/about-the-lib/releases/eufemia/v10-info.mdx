...
v10 of [@dnb/eufemia](https://www.npmjs.com/package/@dnb/eufemia) contains _breaking changes_. As a migration process from v9, you should follow all of the above guides.
...
The support for Internet Explorer (IE) was removed, as Microsoft formally ended support for IE in June, 2022. Have a look at the updated table about supported [browsers and paltforms](/uilib/usage/#supported-browsers-and-platforms).

With that change, Eufemia will support modern browsers that supports ES6.

1. The helpers `isIE11` and `IS_IE11` where removed.
...
2. If you did import `/components` etc. – then you have to change it to the above:
...
3. If you did import the styles as CSS files, then you have to change it to the above:
