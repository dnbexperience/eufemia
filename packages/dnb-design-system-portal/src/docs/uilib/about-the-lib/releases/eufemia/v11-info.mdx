...
| `<NumberFormat compact value={12345} />`        | `<NumberFormat.Compact value={12345} />`                |
| `<NumberFormat compact="long" value={12345} />` | `<NumberFormat.Compact compact="long" value={12345} />` |
...
- The `currency`, `percent`, `compact`, `phone`, `ban`, `nin` and `org` boolean props have been removed from the shared props surface. They are now implied by the chosen variant.
- `NumberFormat.Currency` defaults `currency` to `true` (i.e. `NOK`). Pass a string like `currency="USD"` to override.
- `NumberFormat.Compact` defaults `compact` to `true`. Pass `compact="long"` or `compact="short"` to pick the style explicitly.
...
`useNumberFormatWithParts` is unchanged at the call site – it still accepts an options object – but internally delegates to `formatCurrency` / `formatPercent` / `formatPlainNumber` based on the passed options.
