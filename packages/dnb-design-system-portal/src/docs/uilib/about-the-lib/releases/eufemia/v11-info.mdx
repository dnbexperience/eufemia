...
> **Using AI to migrate?** Load [v11-migration-reference.mdx](./v11-migration-reference.mdx) for the complete rename reference, and set up the [Eufemia MCP server](/uilib/usage/first-steps/tools/#ai-assistance-and-mcp-server-beta) for live API docs. See [AI agent migration workflow](#ai-agent-migration-workflow) for the optimal approach.
...
If you are an AI agent performing this migration (or a developer using an AI agent), follow this workflow for the most efficient migration:

1. Load **[v11-migration-reference.mdx](./v11-migration-reference.mdx)** — it contains every rename in a single, dense file optimized for AI consumption. Use it instead of reading all per-component sections.
2. Follow the [Step-by-step migration procedure](#step-by-step-migration-procedure) (Phases 1–8) in order.
3. Use the [Find-and-replace safety guide](#find-and-replace-safety-guide) to determine which renames are safe to apply globally vs. which must be scoped to specific components.
4. After each phase, run the verification commands in [Verifying your migration](#verifying-your-migration).
5. Check [Silent failures TypeScript won't catch](#silent-failures-typescript-wont-catch) — these are the changes that compile without errors but break at runtime.
6. For component API details not covered here, use the [Eufemia MCP server](/uilib/usage/first-steps/tools/#ai-assistance-and-mcp-server-beta) to query live documentation.

For per-component details (behavioral changes, removals, code examples), refer to the individual [component sections](#components) below.

<Accordion title="Table of contents">

- [AI agent migration workflow](#ai-agent-migration-workflow)
- [Compact rename reference](#compact-rename-reference)
...
## Compact rename reference

> **This section has been moved to a dedicated file for better separation between human-readable docs and AI-consumable reference data.**

See **[v11-migration-reference.mdx](./v11-migration-reference.mdx)** for the complete compact rename reference — it contains every v10 → v11 rename in dense table format, optimized for AI agents and automated migration tools.

The reference file covers: common property renames, event renames, semantic renames, status value renames, CSS class renames, component-specific property renames, translation key renames, SCSS mixin renames, Eufemia Forms renames, import path changes, theming path changes, and TypeScript type renames.
...
These renames change the property or event name itself, not just the casing. They affect multiple components and require targeted find-and-replace. A naive snake_case → camelCase conversion will produce the **wrong** name for many of these.

For the complete table of 27 semantic renames with scope and wrong-conversion warnings, see the [v11 Migration Reference](./v11-migration-reference.mdx#semantic-renames-not-just-casing--must-use-correct-new-name).

Key examples: `on_show` → `onOpen` (not ~~`onShow`~~), `triangle_position` → `arrowPosition` (not ~~`trianglePosition`~~), `spacing` → `innerSpace={{ block: 'value' }}` (must use object form).

### Import path changes

Several import paths changed or were removed in v11. Update these before changing component APIs — your app won't compile until these are fixed.

For the complete table of import path changes, theming path changes, and removed exports, see the [v11 Migration Reference](./v11-migration-reference.mdx#import-path-changes).
...
Affected components: Autocomplete, Dropdown, DrawerList, DatePicker.
...
| `label_direction`     | `labelDirection`    | Autocomplete, DatePicker, Dropdown, FormLabel, Input, InputMasked, Radio.Group, Textarea, ToggleButton.Group                                                    |
...
| `inner_ref`           | `ref`               | Anchor, Button, Input, InputMasked, Section, Textarea (see [innerRef → ref](#innerref--ref))                                                                    |
...
6. Replace `inputClass` with `inputClassName` on Input and InputMasked.
7. Replace `textareaClass` with `textareaClassName` on Textarea.
...
23. Replace `input_class` / `textarea_class` with `inputClassName` / `textareaClassName` (name change, not just casing).
24. Replace `contentSpacing` with `contentInnerSpace` and `tabsSpacing` with `tabsInnerSpace` — **only on** Tabs.

#### Phase 3: Structural changes (require code modification, not just rename)

25. Replace `spacing` with `innerSpace={{ block: 'value' }}` on Section, Dialog.Body, Drawer.Body. Do NOT use `innerSpace="large"` — it must be an object. See [Section](#section).
26. Replace `Theme.Provider` with `Theme.Context` and `darkBackground` with `surface="dark"`. See [Theme.Provider → Theme.Context](#themeprovider--themecontext).
27. Replace `<FormRow>` with `<Flex.Horizontal align="baseline">` and `<FormRow vertical>` with `<Flex.Vertical>`. Replace `FormRow=` with `formElement=` in Provider config. See [Removal of FormRow and FormSet](#removal-of-formrow-and-formset).
28. Replace `openState="opened"` with `open={true}` and `openState="closed"` with `open={false}` on Modal/Dialog/Drawer.
29. Update `dateFormat` and `returnFormat` strings: `YYYY` → `yyyy`, `DD` → `dd` on DatePicker.
30. Replace `Stat.Amount` with `Stat.Number`, `Stat.Info variant="default"` with `variant="plain"`.
31. Replace Logo `brand`/`variant` props with `svg` prop import pattern. See [Logo](#logo).

#### Phase 4: Import path and module changes

32. Update all changed import paths. See [Import path changes](#import-path-changes).
33. If using Ajv with JSON Schema validation, add `ajvInstance={makeAjvInstance()}` to `Form.Handler`. See [Ajv no longer shipped by default](#ajv-no-longer-shipped-by-default).
34. Replace `InputPassword` import with `Field.Password` from Eufemia Forms. See [InputPassword moved to Field.Password](#inputpassword-moved-to-field-password).
35. Replace `StepsLayout` with `Wizard.Container`, `StepsLayout.Step` with `Wizard.Step`, etc.

#### Phase 5: SCSS changes

36. If you import Eufemia SCSS source files with `@import`, replace with `@use` and namespace your calls. See [SCSS: @import → @use](#scss-import--use).
37. Rename all SCSS mixin references to camelCase. See [SCSS mixin renames](#scss-mixin-renames).
38. Remove `extendFocusRing` and `componentReset` SCSS mixin calls — they have been deleted.

#### Phase 6: TypeScript type updates

39. Update any imported context value types (`AccordionContextProps` → `AccordionContextValue`, etc.). See [TypeScript](#typescript).
40. Update any Props type imports (`Props` → component-prefixed name). See [Props Type Exports](#props-type-exports).
41. Update event handler types to match new typed signatures. See [Typed event handlers](#typed-event-handlers).

#### Phase 7: Eufemia Forms behavioral changes

42. Replace `validator` with `onChangeValidator` on all Field components.
43. Replace `continuousValidation` with `validateContinuously` on all Field components.
44. Update `errorMessages` object keys: `required` → `Field.errorRequired`, `pattern` → `Field.errorPattern`, etc. See [Error handling](#error-handling).
45. Replace `Form.useError` with `Form.useValidation`, `Form.useLocale` with `Form.useTranslation`.
46. Replace `Form.Visibility` props: `withValue` → `hasValue`, `pathValue`/`whenValue` → `visibleWhen`.

#### Phase 8: Verify

47. Run `npx tsc --noEmit` to catch remaining type errors.
48. Run your tests. Update any test selectors that query DatePicker/Expiry `input` elements (now `role="spinbutton"` sections).
49. Search for remaining snake_case patterns using the grep command in [Verifying your migration](#verifying-your-migration).
...
```tsx
import { Input, Dropdown, DatePicker, Section, Button } from '@dnb/eufemia'
import { FormRow, Provider } from '@dnb/eufemia/shared'
...
Affected components: `Input`, `Textarea`, `Autocomplete`, `Dropdown`, `DatePicker`, `Slider`, `ProgressIndicator`, `ToggleButton`, `ToggleButton.Group`, `Radio.Group`, `FormLabel`.
...
- Replace `input_icon` with `icon`. The `inputIcon` prop has been removed — use the `icon` prop instead (defaults to `'loupe'`). ⚠️ Scope: Autocomplete only — other components have their own `icon` prop.
- Replace `opened` with `open`. ⚠️ Scope: Autocomplete, Dropdown, DrawerList, DatePicker, Tooltip, Field.Date only — do not apply globally.
- Replace `align_autocomplete` with `align`.
- Replace `on_show` with `onOpen` and `on_hide` with `onClose` (not just a casing change — the event names changed). ⚠️ Scope: Autocomplete, Dropdown, DrawerList, DatePicker only — GlobalStatus still uses `onShow`/`onHide`.
...
- The deprecated `onStateUpdate` prop has been removed. Use `onChange` instead.
- Replace `clear` with `showClearButton`. ⚠️ Scope: Input and InputMasked only — do not apply globally.
...
- Replace `visible` with `show`. ⚠️ Scope: ProgressIndicator only — do not apply globally.
- Replace `no_animation` with `noAnimation`.
- Replace `indicator_label` with `indicatorLabel`.
- Replace `label_direction` with `labelDirection`.
- Replace `show_label` with `showDefaultLabel`.
- Replace `class` with `className`.
- Replace `children` with `label`. ⚠️ Scope: ProgressIndicator and Checkbox only — `children` is a React universal prop.
...
- Replace `size` with `span`. ⚠️ Scope: Flex.Item and Card only — `size` is used by many non-Eufemia components. The property sets how many columns an item should span in the flex layout.
...
- Replace `status_props` with `statusProps`.
- Replace `status_no_animation` with `statusNoAnimation`.
- Replace `children` with `label`. ⚠️ Scope: Checkbox and ProgressIndicator only — `children` is a React universal prop.
...
- Replace `as` with `element`. ⚠️ Scope: H (heading element) only — `as` is used by styled-components and other libraries.
...
- Replace `accordion` with `mode="accordion"`. ⚠️ Scope: Table only — this is a prop-to-prop+value change, not a simple rename.
...
- Replace `position` with `placement` to match the Popover API. ⚠️ Scope: Tooltip only — do not apply globally.
- Replace `active` with `open`. ⚠️ Scope: Tooltip only — `active` is a common generic prop.
