...
### mergeTranslations type change

The `mergeTranslations` function (exported from `@dnb/eufemia/shared`) now accepts `Array<Record<string, unknown>>` instead of `Array<Record<string, any>>`. If you call this function directly with typed objects that are not assignable to `Record<string, unknown>`, you may need to add a type assertion.
