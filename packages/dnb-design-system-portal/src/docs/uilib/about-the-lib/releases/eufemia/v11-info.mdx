...
24. Replace `spacing` with `innerSpace={{ block: 'value' }}` on Section, Dialog.Body, Drawer.Body. Do NOT use `innerSpace="large"` — it must be an object. See [Section](#section).
25. Replace `Theme.Provider` with `Theme.Context` and `darkBackground` with `surface="dark"`. See [Theme.Provider → Theme.Context](#themeprovider--themecontext).
26. Replace `<FormRow>` with `<Flex.Horizontal align="baseline">` and `<FormRow vertical>` with `<Flex.Vertical>`. Replace `FormRow=` with `formElement=` in Provider config. See [Removal of FormRow and FormSet](#removal-of-formrow-and-formset).
27. Replace `openState="opened"` with `open={true}` and `openState="closed"` with `open={false}` on Modal/Dialog/Drawer.
28. Update `dateFormat` and `returnFormat` strings: `YYYY` → `yyyy`, `DD` → `dd` on DatePicker.
29. Replace `Stat.Amount` with `Stat.Number`, `Stat.Info variant="default"` with `variant="plain"`.
30. Replace Logo `brand`/`variant` props with `svg` prop import pattern. See [Logo](#logo).

#### Phase 4: Import path and module changes

31. Update all changed import paths. See [Import path changes](#import-path-changes).
32. If using Ajv with JSON Schema validation, add `ajvInstance={makeAjvInstance()}` to `Form.Handler`. See [Ajv no longer shipped by default](#ajv-no-longer-shipped-by-default).
33. Replace `InputPassword` import with `Field.Password` from Eufemia Forms. See [InputPassword moved to Field.Password](#inputpassword-moved-to-field-password).
34. Replace `StepsLayout` with `Wizard.Container`, `StepsLayout.Step` with `Wizard.Step`, etc.

#### Phase 5: SCSS changes

35. If you import Eufemia SCSS source files with `@import`, replace with `@use` and namespace your calls. See [SCSS: @import → @use](#scss-import--use).
36. Rename all SCSS mixin references to camelCase. See [SCSS mixin renames](#scss-mixin-renames).
37. Remove `extendFocusRing` and `componentReset` SCSS mixin calls — they have been deleted.

#### Phase 6: TypeScript type updates

38. Update any imported context value types (`AccordionContextProps` → `AccordionContextValue`, etc.). See [TypeScript](#typescript).
39. Update any Props type imports (`Props` → component-prefixed name). See [Props Type Exports](#props-type-exports).
40. Update event handler types to match new typed signatures. See [Typed event handlers](#typed-event-handlers).

#### Phase 7: Eufemia Forms behavioral changes

41. Replace `validator` with `onChangeValidator` on all Field components.
42. Replace `continuousValidation` with `validateContinuously` on all Field components.
43. Update `errorMessages` object keys: `required` → `Field.errorRequired`, `pattern` → `Field.errorPattern`, etc. See [Error handling](#error-handling).
44. Replace `Form.useError` with `Form.useValidation`, `Form.useLocale` with `Form.useTranslation`.
45. Replace `Form.Visibility` props: `withValue` → `hasValue`, `pathValue`/`whenValue` → `visibleWhen`.

#### Phase 8: Verify

46. Run `npx tsc --noEmit` to catch remaining type errors.
47. Run your tests. Update any test selectors that query DatePicker/Expiry `input` elements (now `role="spinbutton"` sections).
48. Search for remaining snake_case patterns using the grep command in [Verifying your migration](#verifying-your-migration).
