...
4. Replace the `size` prop with `span` on `Flex.Item` and `Card` components.
5. If you `@use` Eufemia SCSS source files and reference internal mixins, rename them to camelCase (see [SCSS mixin renames](#scss-mixin-renames)).
6. Replace `size="default"` with the equivalent standard value or remove the prop (see [Standardized size values](#standardized-size-values)).
...
- The `size` prop has been standardized across all components. The `'default'` value has been removed (replaced with `'small'` or `'medium'` depending on context), `FormLabel`'s `'basis'` was renamed to `'small'`, and `Icon`'s `'default'` was renamed to `'basis'`. See [Standardized size values](#standardized-size-values).
...
- Replace `forceActive` with `forceOpen`.
...
The `size` prop has been standardized across all components to use consistent T-shirt sizing (`small`, `medium`, `large`). The ambiguous `'default'` value has been removed, and `FormLabel`'s `'basis'` has been renamed to `'small'`. `Icon` renames `'default'` to `'basis'` to align with the typography sizing scale.

### Migration

**Components where `size="default"` should be removed** (the default appearance is what you get when no `size` prop is specified):
...
**Components where `size="default"` becomes `size="small"`:**
...
**Components where `size="default"` becomes `size="medium"`:**

- `FormStatus`
- `Tooltip`

```diff
- <FormStatus size="default">…</FormStatus>
+ <FormStatus size="medium">…</FormStatus>
```

**FormLabel: `size="basis"` becomes `size="small"`:**

```diff
- <FormLabel size="basis" />
+ <FormLabel size="small" />
```

**Icon: `size="default"` becomes `size="basis"`:**

```diff
- <Icon size="default" icon={myIcon} />
+ <Icon size="basis" icon={myIcon} />
```

The CSS class `dnb-icon--default` has been renamed to `dnb-icon--basis`.
