...
`MultiInputMask` is no longer documented as part of the public `InputMasked` docs.

If you rely on `MultiInputMask`, treat it as an internal or unsupported API and plan to migrate away from it before upgrading.
...
### Event callback type renames

Callback parameter types have been standardized to use `...Event` suffix (matching the existing convention used by Input, Radio, Button, etc.):

| Before                       | After                     |
| ---------------------------- | ------------------------- |
| `CheckboxOnChangeParams`     | `CheckboxChangeEvent`     |
| `CheckboxOnClickParams`      | `CheckboxClickEvent`      |
| `SliderOnChangeParams`       | `SliderChangeEvent`       |
| `SwitchOnChangeParams`       | `SwitchChangeEvent`       |
| `SwitchOnClickParams`        | `SwitchClickEvent`        |
| `AutocompleteOnClearParams`  | `AutocompleteClearEvent`  |
| `AutocompleteOnTypeParams`   | `AutocompleteTypeEvent`   |
| `AutocompleteOnFocusParams`  | `AutocompleteFocusEvent`  |
| `AutocompleteOnBlurParams`   | `AutocompleteBlurEvent`   |
| `AutocompleteOnChangeParams` | `AutocompleteChangeEvent` |
| `AutocompleteOnSelectParams` | `AutocompleteSelectEvent` |
| `CloseHandlerParams`         | `ModalCloseEvent`         |
