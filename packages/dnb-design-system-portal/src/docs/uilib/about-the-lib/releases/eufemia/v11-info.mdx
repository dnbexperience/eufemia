...
| `children` → `label`             | Checkbox, ProgressIndicator | On Checkbox, `children` is a silent failure  |
| `as` → `element`                 | H (heading element)         | —                                            |
...
15. Replace `children` with `label` — **only on** Checkbox, ProgressIndicator. On Checkbox, `children` no longer renders (silent failure — see [Silent failures](#silent-failures-typescript-wont-catch)). On ProgressIndicator, `children` still works as an alias but `label` is preferred.
...
- Replace `direction` in `<Dl>` with `layout`. Values `'vertical'` and `'horizontal'` are unchanged; a new `'grid'` value is also available.
