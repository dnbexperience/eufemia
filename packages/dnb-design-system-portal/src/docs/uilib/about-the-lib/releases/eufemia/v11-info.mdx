...
All component, element, extension, and fragment entry points now include `"use client"` directives. This marks Eufemia components as client components in React Server Component (RSC) environments such as the Next.js App Router. No action is needed — existing consumers are unaffected, but RSC-based apps can now import Eufemia components without manually adding client boundaries.
