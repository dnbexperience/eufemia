...
Affected components: `Input`, `Textarea`, `Autocomplete`, `Dropdown`, `DatePicker`, `Slider`, `ProgressIndicator`, `ToggleButton`, `ToggleButton.Group`, `Radio.Group`.
...
1. Install `ajv` and `ajv-errors` as dependencies in your project:

```bash
npm install ajv ajv-errors
```

2. Import `makeAjvInstance` from the dedicated path instead of the barrel export:
...
import { makeAjvInstance } from '@dnb/eufemia/extensions/forms/utils/ajv'
...
### JSONSchemaType removed from public API

**Breaking Change**: The `JSONSchemaType` type from ajv is no longer re-exported from `@dnb/eufemia/extensions/forms`. Use the `JSONSchema` type instead, which accepts any valid JSON Schema object.

If you need the strict `JSONSchemaType` from ajv, import it directly:

```tsx
import type { JSONSchemaType } from 'ajv/dist/2020.js'
```

### FormError messageValues type widened

**Breaking Change**: `FormError`'s `messageValues` type changed from `Record<string, string>` to `Record<string, string | number>` to correctly reflect that validation rules like `minLength`, `maximum`, etc. produce numeric values.

This is only a breaking change if you have code that strictly types `messageValues` as `Record<string, string>`.
