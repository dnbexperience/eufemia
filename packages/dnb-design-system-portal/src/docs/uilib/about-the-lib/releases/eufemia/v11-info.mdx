...
`Theme.Provider` has been renamed to `Theme.Context` to align with React 19's context-as-provider pattern. Use `Theme.Context` when you need to provide theme without a wrapper element (e.g. on dark surfaces):

**Before:**

```tsx
<Theme.Provider surface="dark">
