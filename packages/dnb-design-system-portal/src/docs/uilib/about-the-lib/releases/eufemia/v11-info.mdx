...
```bash
grep -rn 'on_\|status_\|label_\|icon_\|_class\b\|inner_ref\|_state\|_position\|_button\|_text\|_sr_\|_title\|_format\|_mask\|_currency\|_number\|_percent' \
  --include='*.tsx' --include='*.ts' src/
```

> **Note:** This grep catches most snake_case patterns but is not exhaustive. Semantic renames like `opened` → `open`, `prerender` → `keepInDOM`, `clear` → `showClearButton`, and `expandBehaviour` → `expandBehavior` require manual review using the [Semantic renames](#semantic-renames-not-just-casing) table. Always run `npx tsc --noEmit` as the primary verification step.
...
#### 6. Checkbox `children` silently ignored

Checkbox no longer uses `children` for the label text. Because Checkbox's type extends `React.HTMLProps<HTMLInputElement>`, passing `children` still compiles — but the label won't render.
...
```bash
grep -rn '<Checkbox[^>]*>' --include='*.tsx' --include='*.ts' src/ | grep -v 'label='
```
