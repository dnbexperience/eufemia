...
**Migration:** If you relied on the space between the country code and the phone number, update your code to handle the spaceless format. The `additionalArgs` second argument of `onChange` still provides `countryCode` and `phoneNumber` separately. Space-separated values like `"+47 12345678"` are still accepted as input and silently normalized to E.164.
