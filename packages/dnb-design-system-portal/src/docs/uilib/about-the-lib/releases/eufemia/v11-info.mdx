...
For apps with light Eufemia usage, this minimal checklist covers the most common changes. For comprehensive coverage, follow the full [Step-by-step migration procedure](#step-by-step-migration-procedure) instead.

1. Upgrade React and React DOM to v19.
2. Install `@dnb/eufemia@11`.
3. Find-and-replace all snake_case props and events (e.g. `on_change` → `onChange`, `status_state` → `statusState`).
4. Replace `innerRef` with `ref`.
5. Replace status values: `'info'` → `'information'`, `'warn'` → `'warning'`.
6. Run `npx tsc --noEmit` and fix any remaining type errors.
7. Run your test suite — update DatePicker/Expiry test selectors (`input` → `[role="spinbutton"]`).
8. Review [Silent failures TypeScript won't catch](#silent-failures-typescript-wont-catch).
...
56. Run your tests. Pay special attention to:
    - DatePicker/Expiry selectors: `input` elements are now `role="spinbutton"` sections (use `document.querySelector('[role="spinbutton"]')` or `.dnb-segmented-field__section`).
    - CSS class assertions: renamed classes like `--opened` → `--open`, `--visible` → `--show`, `--contrast` → `--surface-dark` (see [CSS class renames](#cross-component-prop-patterns)).
    - Event callback destructuring: properties like `days_between`, `is_valid` are now camelCase (see [Silent failures](#silent-failures-typescript-wont-catch)).
