...
import { Accordion as AccordionComponent } from '@dnb/eufemia/src'
...
<AccordionComponent title="Table of contents">
...
- [`createSpacingClasses` replaced with `applySpacing`](#createspacingclasses-replaced-with-applyspacing)
...
Note: GlobalStatus has both `onOpen`/`onClose` and `onShow`/`onHide`, but they have distinct semantics: `onOpen` fires when GlobalStatus first appears, `onShow` fires on first appearance and on every content update, `onClose` fires when the component disappears (animation end), and `onHide` fires when the user triggers close. Do not rename `onShow`/`onHide` to `onOpen`/`onClose` on GlobalStatus — they are separate events.
...
| `innerRef` / `inner_ref` | `ref`               | All components that accepted `innerRef` (see [innerRef → ref](#innerref--ref) for the full list)                                                                |
...
9. Replace `on_show` with `onOpen` and `on_hide` with `onClose` — **only on** Autocomplete, Dropdown, DrawerList, DatePicker. _(Not just a casing change — the name changed semantically. GlobalStatus has both `onShow`/`onHide` and `onOpen`/`onClose` with different semantics — do not rename them.)_
...
> **Note:** `on_state_update` (and its camelCase form `onStateUpdate`) is **not** in this list because it has been removed, not renamed. If you used either form, replace it with `onChange` instead.
...
The `labelDirection` prop now defaults to `vertical` instead of `horizontal` on all base form components. This means labels are rendered above the input by default. If you relied on the horizontal default, explicitly set `labelDirection="horizontal"`.
...
- Replace `place_marker_before_content` with `placeMarkerBeforeContent`.
