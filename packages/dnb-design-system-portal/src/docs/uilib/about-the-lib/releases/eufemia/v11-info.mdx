...
The following deprecated vendor prefixes and CSS properties have been removed from Eufemia's core styles:

- `-moz-tab-size` — the unprefixed `tab-size` is supported in all modern browsers.
- `-webkit-overflow-scrolling: touch` — momentum scrolling is the default since iOS 13.
- `clip: rect(0,0,0,0)` — replaced with `clip-path: inset(100%)`, as `clip` has been removed from the CSS spec.

If you have custom CSS that relied on Eufemia providing these vendor-prefixed rules, add them to your own styles.
