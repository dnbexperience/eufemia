...
- The `moreMenu` prop has been removed. Use the new [Menu](/uilib/components/menu/) component instead, or use `preventSelection` with `title={null}` for popup-style behavior.
- Replace `opened` with `open`.
