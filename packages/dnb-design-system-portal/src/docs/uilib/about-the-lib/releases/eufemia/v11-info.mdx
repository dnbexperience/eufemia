...
The `InputPassword` component has been moved to `Field.Password`, and is now a part of the `Forms` `extensions`. Change your import statement from `import InputPassword from '@dnb/eufemia/components/input/InputPassword'` to `import { Field } from '@dnb/eufemia/extensions/forms'`
