...
- Replace `disable_filter` with `search={{ filter: false }}`.
- Replace `disable_reorder` with `search={{ reorder: false }}`.
- Replace `disable_highlighting` with `search={{ highlight: false }}`.
...
**Migration:** If you relied on the space between the country code and the phone number, update your code to handle E.164 format. The `additionalArgs` second argument of `onChange` still provides `countryCode` and `phoneNumber` separately. Values with spaces (e.g. `"+47 12345678"`) are no longer accepted and will not be displayed.
