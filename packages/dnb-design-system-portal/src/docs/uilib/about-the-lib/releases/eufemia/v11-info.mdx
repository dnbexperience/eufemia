...
- `show_guide` has been removed in v11. If you need to communicate an expected format, pass an example using the regular `placeholder` prop (e.g. `placeholder="00 00 00"`) or provide helper text next to the field.
- `keep_char_positions` has been removed in v11.
- `pipe` has been removed in v11.
- `MaskFunction` (function-based masks) has been removed in v11. Maskito only supports array masks (`Array<string | RegExp>`) and RegExp masks. Convert any custom mask functions to array-based masks.
- The `{ mask: ... }` object form for masks has been removed in v11. Use array masks directly instead of wrapping them in an object.
- The addon helpers `createNumberMask` and `emailMask` have been removed in v11. The following imports no longer work and should be deleted or replaced:
