...
The `"typings"` field in package.json has been renamed to `"types"`. This is the modern, recommended field name recognized by TypeScript. If you have tooling that reads the `"typings"` field from `@dnb/eufemia/package.json`, update it to read `"types"` instead.
