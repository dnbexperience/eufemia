...
> **Using AI to migrate?** Set up the [Eufemia MCP server](/uilib/usage/first-steps/tools/#ai-assistance-and-mcp-server-beta) to give your AI agent access to the full v11 API documentation alongside this migration guide. See [AI agent migration workflow](#ai-agent-migration-workflow) for the optimal approach.
...
1. Load the [Compact rename reference](#compact-rename-reference) section — it contains every rename in a single, dense section. Use it instead of reading all per-component sections.
...
This section lists every v10 → v11 rename in a single, dense reference. Use it for automated migration or quick lookups without reading the full per-component sections. For behavioral changes, removals, and code examples, see the individual [component sections](#components).
...
These renames follow the same pattern everywhere they appear. Apply them as global find-and-replace within Eufemia component JSX.
...
These renames change the name itself, not just the casing. A naive snake_case → camelCase conversion will produce the **wrong** name.
...
These properties are unique to a single component and only listed once here.

#### Section

| Old                      | New             |
| ------------------------ | --------------- |
| `variant` value `'info'` | `'information'` |

#### Autocomplete (unique props)
...
#### Radio / Radio.Group (unique props)

| Old                              | New               |
| -------------------------------- | ----------------- |
| `layout_direction` (Radio.Group) | `layoutDirection` |

#### Textarea (unique props)
...
#### ProgressIndicator (unique props)
...
#### Modal / Dialog / Drawer (unique props)
...
For the full list of 60+ type renames across context types, stuttering names, plural-to-singular, and non-prefixed types, see the [TypeScript](#typescript) section. The most commonly encountered ones:
