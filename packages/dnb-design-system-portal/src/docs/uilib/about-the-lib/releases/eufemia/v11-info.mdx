...
6. Replace the state/variant value `'info'` with `'information'` in all components (e.g., `state="info"` → `state="information"`, `variant="info"` → `variant="information"`).
7. If you import Eufemia SCSS source files with `@import`, replace with `@use` and namespace your mixin/variable calls (see [SCSS: @import → @use](#scss-import--use)).
8. Replace the `size` prop with `span` on `Flex.Item` and `Card` components.
9. If you `@use` Eufemia SCSS source files and reference internal mixins, rename them to camelCase (see [SCSS mixin renames](#scss-mixin-renames)).
...
16. If you import event callback parameter types, rename `On{Action}Params` to `{Action}Event` (e.g., `CheckboxOnChangeParams` → `CheckboxChangeEvent`). Also rename `ButtonOnClick` → `ButtonClick` and `SwitchOnChange` → `SwitchChange`. See each component section below for specific renames.
...
- Replace `AutocompleteOnClearParams` with `AutocompleteClearEvent`.
- Replace `AutocompleteOnTypeParams` with `AutocompleteTypeEvent`.
- Replace `AutocompleteOnFocusParams` with `AutocompleteFocusEvent`.
- Replace `AutocompleteOnBlurParams` with `AutocompleteBlurEvent`.
- Replace `AutocompleteOnChangeParams` with `AutocompleteChangeEvent`.
- Replace `AutocompleteOnSelectParams` with `AutocompleteSelectEvent`.
- Replace `AutocompleteOnSubmitParams` with `AutocompleteSubmitEvent`.
...
- Replace `CheckboxOnChangeParams` with `CheckboxChangeEvent`.
- Replace `CheckboxOnClickParams` with `CheckboxClickEvent`.
...
- Replace `SwitchOnChangeParams` with `SwitchChangeEvent`.
- Replace `SwitchOnClickParams` with `SwitchClickEvent`.
- Replace `SwitchOnChange` with `SwitchChange`.
...
- Replace `ButtonOnClick` with `ButtonClick`.
...
- Replace `OnChangeEventProps` with `SliderChangeEvent`.
- Replace `SliderOnChangeParams` with `SliderChangeEvent`.
