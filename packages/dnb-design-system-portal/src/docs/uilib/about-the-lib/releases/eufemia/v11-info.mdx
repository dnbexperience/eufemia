...
⚠️ These renames change the name itself, not just the casing. A naive snake_case → camelCase conversion will produce the **wrong** name. Each must be scoped to the listed components — see the [find-and-replace safety guide](#find-and-replace-safety-guide) for collision risks.
