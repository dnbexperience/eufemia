...
- Find property `onDelete` and replace with `onClick`, add property `variant='removable'`.

## Definition lists

- Find `direction` in `<Dl>` and replace with `layout`.

## Removal of passing down properties to BreadcrumbItem's span

We don't think this has been used for anything other than passing down `data-testid`'s for testing. We believe the potential side effects of passing down properties to this span is greater than the advantages it gives for those who want to test this span using data-testid as their way of selecting the span.
...
- `rowGap` no longer accepts value `true`. Replace `true` with `undefined` or simply remove the property to get the same behavior.
...
- `on_item_render` removed. No longer has any other function than each step's `title` property.
- step item `on_render` removed. No longer has any other function than the `title` property.
- `sidebarId` removed. No longer has any sidebar. If an id is needed, use the `id` property.
