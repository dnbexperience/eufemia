...
- ~~Replace `action_menu` with `actionMenu`.~~ The `actionMenu` (and `action_menu`) prop has been removed from DrawerList. Use the new [Menu](/uilib/components/menu/) component instead.
...
The component now emits values in pure E.164 format without spaces: `"+4712345678"` instead of `"+47 12345678"`. This applies to `onChange`, `transformOut`, the data context, and pattern/schema validation.

**Migration:** If you relied on the space between the country code and the phone number, update your code to handle the spaceless format. The `additionalArgs` second argument of `onChange` still provides `countryCode` and `phoneNumber` separately.
...
The component now auto-detects country codes from spaceless E.164 values like `"+4712345678"` and `00`-prefixed values like `"004712345678"`. No `transformIn` is needed for standard formats.

#### `omitSpaceSeparator` prop removed

The `omitSpaceSeparator` prop has been removed since E.164 (without spaces) is now the default output format.

#### `detectCountryCode` utility

A new `detectCountryCode` utility is exported from `@dnb/eufemia/extensions/forms` for detecting E.164 country codes from phone number strings.
