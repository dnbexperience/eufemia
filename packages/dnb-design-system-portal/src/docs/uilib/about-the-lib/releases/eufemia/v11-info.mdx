...
- prop `no_animation` renamed to `noAnimation`
- prop `label_direction` renamed to `labelDirection`
- prop `show_label` renamed to `showDefaultLabel`
- prop `on_complete` renamed to `onComplete`

Removed props:

- prop `class` removed, use prop `className`
- prop `children` removed, use prop `label`
