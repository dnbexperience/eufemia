...
- Each variant only accepts the props relevant to it. `NumberFormat.Number` and `NumberFormat.Percent` do **not** accept `currency`, `currencyDisplay`, or `currencyPosition` — use `NumberFormat.Currency` for those. Passing currency-related props to `NumberFormat.Number` will cause a TypeScript error.
...
| `formatPlainNumber(value)`           | `formatNumber(value)`                        |
