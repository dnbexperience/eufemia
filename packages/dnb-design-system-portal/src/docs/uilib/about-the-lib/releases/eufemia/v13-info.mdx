...
Several exported type names now carry their component prefix, to make them consistent and unambiguous across the library. These are type-only renames — the runtime API (components, hooks and constants) is unchanged. Update `import type` statements and type references as follows:

```diff
-import type { DisplayPickerEvent } from '@dnb/eufemia/components/DatePicker'
+import type { DatePickerDisplayEvent } from '@dnb/eufemia/components/DatePicker'
```

| Component    | Before                 | After                          |
| ------------ | ---------------------- | ------------------------------ |
| DatePicker   | `DisplayPickerEvent`   | `DatePickerDisplayEvent`       |
| FormStatus   | `ErrorIconProps`       | `FormStatusErrorIconProps`     |
| FormStatus   | `WarnIconProps`        | `FormStatusWarnIconProps`      |
| FormStatus   | `InfoIconProps`        | `FormStatusInfoIconProps`      |
| FormStatus   | `MarketingIconProps`   | `FormStatusMarketingIconProps` |
| Heading      | `InternalHeadingLevel` | `HeadingInternalLevel`         |
| Icon         | `DefaultIconSizes`     | `IconDefaultSizes`             |
| Icon         | `ValidIconType`        | `IconValidType`                |
| Icon         | `ValidIconNumericSize` | `IconValidNumericSize`         |
| Logo         | `Svg`                  | `LogoSvgInput`                 |
| Logo         | `SvgComponent`         | `LogoSvgComponentType`         |
| Logo         | `CustomLogoSvg`        | `LogoCustomSvg`                |
| NumberFormat | `NumberFormatter`      | `NumberFormatFormatter`        |
| NumberFormat | `BankAccountType`      | `NumberFormatBankAccountType`  |

The `Icon` renames affect only the types. The `ValidIconType` and `DefaultIconSizes` runtime constants keep their names.
