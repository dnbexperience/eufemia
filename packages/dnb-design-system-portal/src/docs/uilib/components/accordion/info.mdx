...
There is the `Accordion.Provider` and `Accordion.Group`. They are technically the same, except that `Accordion.Group` will provide automatically a unique `group` id, and with that, make all the nested accordions work together and close each other.
