...
### Show a progress indicator during initial data load

While fetching the initial data, you can display a `ProgressIndicator` inside the input by passing it to the `icon` property and toggling it with your own loading state. The field is disabled until the data is ready.
