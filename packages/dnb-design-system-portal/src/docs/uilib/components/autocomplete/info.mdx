...
1. The third word will match inside a word (can be changed with `search={{ inWordIndex: number }}`)
1. Case-insensitive
...
Numbers are often different from a word filter. You can use `search={{ matchNumbers: true }}` to enable number-specialized filtering. See examples in the demos.
