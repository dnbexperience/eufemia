...
| `variant`                               | _(optional)_ Force component to use same variant on all screen sizes. Options: `single` \| `multiple` \| `collapse`. If no variant is defined, it will change presentation based on screen size.        |
