...
Use `variant="nested"` to apply nested card styling (thinner border, lighter color) without actually nesting the card inside another card.

<Examples.NestedVariant />
...
<VisibleWhenVisualTest>
  <Examples.WithTableOutline />
  <Examples.InColoredSection />
</VisibleWhenVisualTest>

## With `backgroundColor` and `outline`

<Examples.CustomBackgroundColorAndOutline />
