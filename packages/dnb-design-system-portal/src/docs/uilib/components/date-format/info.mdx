...
### `getOsloDateISO` helper

When you need an ISO date string that always reflects the current day in `Europe/Oslo`, pull in the helper directly:

```ts
import { getOsloDateISO } from '@dnb/eufemia/components/date-format/DateFormatUtils'
getOsloDateISO() // -> e.g. "2025-11-24" regardless of the runtime timezone
```
