...
The `correctInvalidDate` property is not on by default, as it can be confusing to screen reader users, when the value they type in changes, for seemingly no reason to them.
For built in validation, use [Field.Date](/uilib/extensions/forms/feature-fields/Date/#date-limit-validation)
