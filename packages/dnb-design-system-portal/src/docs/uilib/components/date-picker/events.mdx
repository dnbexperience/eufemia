...
```js
{
  ...
  partialStartDate:  '' | 'date as `returnFormat` | `yyyy-MM-dd`' /* Always available if `range` is `true` */
  partialEndDate: '' | 'date as `returnFormat` | `yyyy-MM-dd`' /* Available if `range` is `true` */
  days_between: number,
