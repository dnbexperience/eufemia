...
Use the `status` property to display a status message as a [FormStatus](/uilib/components/form-status) inside the dialog. Defaults to the `error` state. Use `statusState` to change it to `warning`, `information`, etc.
