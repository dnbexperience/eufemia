...
The Dropdown component can be used as an **action button** by setting `actionMenu={true}`. In mobile view, the title/text will be hidden, showing only the icon, and the DrawerList will open from the bottom of the browser.

## Menu button

The Dropdown component can be used as a **menu button** by setting `moreMenu={true}`, which displays the [more](/icons/primary#a-list-of-all-primary-icons) icon (appears as dots). You can also use `preventSelection={true}` together with an empty title `title=""` and `aria-label="Choose an item"`.
