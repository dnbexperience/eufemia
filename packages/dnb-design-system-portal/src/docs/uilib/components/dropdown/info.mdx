...
The Dropdown component can easily be used as a so called **menu button** by setting the property `more_menu={true}` which shows then the [more](/icons/primary#icon-more) icon, appears as dots. You also could use `preventSelection={true}` together with an empty title `title=""` and `aria-label="Choose an item"`.
