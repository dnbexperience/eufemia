...
The Dropdown component can be used as an **action button** by setting `mode="action-menu"`. In mobile view, the title/text will be hidden, showing only the icon, and the DrawerList will open from the bottom of the browser.

## Menu button

The Dropdown component can be used as a **menu button** by setting `mode="more-menu"`, which displays the [more](/icons/primary#a-list-of-all-primary-icons) icon (appears as dots). You can also use `mode="prevent"` together with an empty title `title=""` and `aria-label="Choose an item"`.

## Accessibility

Both the Action Menu and the Menu Button (and if `mode="prevent"`), the Dropdown will use `role="menu"`, instead of `role="menuitems"` for better screen reader support.
