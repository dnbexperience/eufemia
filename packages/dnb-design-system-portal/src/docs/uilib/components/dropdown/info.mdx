...
The Dropdown component can be used as a **menu button** by setting `moreMenu={true}`, which displays the [more](/icons/primary#icon-more) icon (appears as dots). You can also use `preventSelection={true}` together with an empty title `title=""` and `aria-label="Choose an item"`.
