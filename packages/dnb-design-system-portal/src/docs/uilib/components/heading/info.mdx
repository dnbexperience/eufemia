...
Heading leveling is handled synchronously. You can isolate part of the tree with `<Heading.Level ...`, which also lets you add headings asynchronously inside that boundary. Several level contexts can be nested.
...
They are optional in client-only apps and can simplify asynchronous or isolated content.

<HeadingInfoLevel />

#### Server rendering

Provider-free level handling is shared by headings in the browser. During server rendering, wrap each independently rendered tree in a level provider so one request cannot affect the next:
