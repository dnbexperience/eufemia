...
#### Using the `filled` prop

Set `filled` directly on a single `Icon` to fill it. This always applies regardless of the icon:

```jsx
<Icon icon={star} filled />
```

#### Using `Icon.Filled` (context provider)

Wrap multiple components with `Icon.Filled` to fill all supported icons within that subtree. Only icons in the [supported list](/icons#filled-icons) will be filled — unsupported icons are left as-is:

```jsx
<Icon.Filled>
  <Icon icon={star} /> {/* filled — supported */}
  <Icon icon="chevron_right" /> {/* not filled — not supported */}
  <Button icon={bell} /> {/* filled — supported */}
</Icon.Filled>
```

See the [Icon Library](/icons#filled-icons) for the full list of icons that support the filled variant.
