...
You will either create your own mask, or use one of the provided ones. There are also masks which change based on different [locales](/uilib/components/input-masked/info?fullscreen#mask-based-on-locale) (`as_currency` or `as_number`).
...
If you use `as_currency` or `as_number` you have to always send in a clean number without any mask (`value="1234.50"`):
