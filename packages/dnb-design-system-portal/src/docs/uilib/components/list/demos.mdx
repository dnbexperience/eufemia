...
Use `List.Card` to wrap a list inside a [Card](/uilib/components/card) with built-in styling. Add `List.Card.ScrollView` for a scrollable area with a `maxHeight`.
