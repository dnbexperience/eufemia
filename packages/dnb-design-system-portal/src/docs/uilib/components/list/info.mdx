...
CustomRow._supportsSpacingProps = true

render(
  <List.Container>
    <CustomRow title="Row one" />
    <CustomRow title="Row two" />
  </List.Container>
)
```

Keep item components as direct children of `List.Container`. Grouping rows inside a wrapper (for example via `Flex.withChildren`) nests them in an extra `<ul>` and produces invalid list markup.
