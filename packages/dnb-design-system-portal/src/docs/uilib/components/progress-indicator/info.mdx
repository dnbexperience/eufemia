...
- The component uses `role="progressbar"` for both determinate and indeterminate states.
- Determinate states expose the current progress through `aria-valuenow`. Indeterminate states omit this attribute so screen readers announce an unknown progress value.
