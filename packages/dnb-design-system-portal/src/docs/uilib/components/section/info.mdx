...
Under the hood, it uses a couple of tricks to achieve an infinite fullscreen size. You do not need to do anything more than you normally would with your content. The background from the Section component will extend beyond a `max-width` when enabled with the `breakout` property.
