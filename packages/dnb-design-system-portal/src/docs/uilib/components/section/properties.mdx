...
| `breakout`                              | _(optional)_ use `true` to enable a fullscreen breakout look. Defaults to `true`.                                                                              |
| `outline`                               | _(optional)_ define a custom border color. If `true` is given, `color-black-8` is used. Use a Eufemia color.                                                   |
| `roundedCorner`                         | _(optional)_ use `true` to enable rounded corners (border-radius). Defaults to `false`.                                                                        |
| `backgroundColor`                       | _(optional)_ define a custom background color, instead of a variant. Use a Eufemia color.                                                                      |
| `textColor`                             | _(optional)_ define a custom text color to compliment the backgroundColor. Use a Eufemia color.                                                                |
