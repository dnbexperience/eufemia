...
- `Stat.Trend` renders explicit plus (`+`) and minus (`−`) indicators with red/green background states and screen-reader text.
...
- Negative values follow [NumberFormat's minus-sign conventions](/uilib/components/number-format/#minus-signs), including normalization to the Unicode minus sign (`U+2212`, −) for visual and screen-reader output.
