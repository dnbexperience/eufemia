...
| `on_render`    | _(optional)_ callback function to manipulate or wrap a certain item. Has to return a React Node. Recieves parameters `{ StepItem, element, attributes, props, context }` |
| `on_click`     | _(optional)_ event function that gets invoked once the users clicks on the active item. Recieves parameters `{ event, item, current_step, currentStep }`                 |
