...
### Table with a sticky header and an outline

The `outline` is drawn on top of the sticky header, so the frame stays unbroken while the header is stuck.

<Examples.StickyOutline />
