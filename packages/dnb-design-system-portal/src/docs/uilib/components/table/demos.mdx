...
When the `selected` prop is provided (either `true` or `false`), the cell button will be announced as a toggle button by screen readers, conveying its pressed state.
