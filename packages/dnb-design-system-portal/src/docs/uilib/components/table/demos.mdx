...
To adjust the border colors accordingly, pass the `highlightPlugin` via the `plugins` prop on `<Table>`.

```tsx
import Table, { highlightPlugin } from '@dnb/eufemia/components/Table'

render(<Table plugins={[highlightPlugin]}>...</Table>)
```
