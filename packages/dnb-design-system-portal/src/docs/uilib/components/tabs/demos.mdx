...
import {
  ...
  TabsExampleHorizontallyAligned,
  TabsExampleMaxWidth,
  TabsExampleReachRouterNavigation,
  TabsNoBorder,
  TabsNoBreakout,
  TabsExampleKeepInDOM,
  TabsSingleChildrenReactElement,
  TabsSingleElementData,
  TabsAlignProperty,
  TabsExampleWithBadgeNotification,
  TabsExampleDisabledWithTooltip,
...
### Disabled tabs with a tooltip

Use `tooltip` together with `disabled` to explain why a tab is unavailable. The tab stays hoverable, but it cannot be selected by clicking, the scroll navigation buttons or the keyboard.
