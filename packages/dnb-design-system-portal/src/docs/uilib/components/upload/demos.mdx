...
import {
  UploadBasic,
  UploadBasicCompactVariant,
  UploadPrefilledFileList,
  UploadRemoveFile,
  UploadSingleFile,
  UploadFilesAmountMessage,
  UploadIsLoading,
  UploadErrorMessage,
...
The `compact` variant displays only the barebone needed for the Upload component.
Which is the upload button, list of uploaded files, and optionally the `title` as label (can be removed by providing the value `false`).
It does not display info about the `filesAmountLimit`, `fileMaxSize`, `acceptedFileTypes` or the additional `text` which the `normal` (default) variant does.
