...
import {
  ...
  UploadFileMaxSizeBasedOnFileFormat,
...
You can pass an array of objects `Array<{ fileType, fileMaxSize }>` to `acceptedFileTypes`, to define file max size for each file format/type.
You specify `fileMaxSize` for a given `fileType`, as a `number` in MB. To disable(not display a fileMaxSize) use either `0` or `false`. If you don't set a `fileMaxSize` for a given `fileType`, the value of `fileMaxSize` provided to the Upload component will be used(Defaults to 5 MB).
