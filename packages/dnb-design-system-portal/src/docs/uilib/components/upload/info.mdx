...
The `compact` variant displays less information than the `normal` variant, and aims to only display the most necessary for the user to be able to upload a file.
Which is the:
