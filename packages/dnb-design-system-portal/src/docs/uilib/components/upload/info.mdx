...
Each file item displays as a clickable link with it's original file name, which opens the file source in a new browser tab.

In some situations, it's more suitable to have each link download the file instead of opening it in the browser. To achieve this, set the `download={true}` property on the Upload component.
