...
const { withConfig } = Connectors.createContext({
  fetchConfig: {
    url: (value, { country }) => {
      return `[YOUR-API-URL]/.../${country}/.../${value}`
    },
  },
})
```
...
The Bring API for PostalCode supports the [following countries](https://developer.bring.com/api/postal-code/#supported-countries):
...
### To autocomplete a city name based on a postal code
