...
- `reduceToVisibleFields` will reduces the given data set to only contain the visible fields (mounted fields).
...
const MyForm = () => {
  return (
    <Form.Handler
      onSubmit={(
        data,
        { filterData, reduceToVisibleFields, resetForm, clearData },
      ) => {
        resetForm()
        clearData()

        const myData = reduceToVisibleFields(filterData(myFilter))
      }}
