...
- `transformData` Will call the given function for each data path. The returned value will be used as the new value.
...
The `transformData` function will call the given function for each data path. The returned value will be used as the new value.
...
**displayValue** will be `undefined` for most fields.

- It will return the displayed/active option content for [Selection](/uilib/extensions/forms/base-fields/Selection/) fields as a string (or React.ReactNode).
- It will return the displayed/active options content for [ArraySelection](/uilib/extensions/forms/base-fields/ArraySelection/) fields as an array that contains a string (or React.ReactNode).
- It will return the displayed text value for [Toggle](/uilib/extensions/forms/base-fields/Toggle/) field as a string (or React.ReactNode).
- It will return the displayed text value for [Boolean](/uilib/extensions/forms/base-fields/Toggle/) field as a string.
- It will return the displayed value (including the mask) for [String](/uilib/extensions/forms/base-fields/String/) field as a string.
- It will return the displayed value (including the mask) for [Number](/uilib/extensions/forms/base-fields/Number/) field as a string.
- It will return the displayed value (including the mask) for [Currency](/uilib/extensions/forms/feature-fields/Currency/) field as a string.
- It will return the displayed value (including the mask) for [PhoneNumber](/uilib/extensions/forms/feature-fields/PhoneNumber/) field as a string.
- It will return the selected country name for [SelectCountry](/uilib/extensions/forms/feature-fields/SelectCountry/) field as a string.
