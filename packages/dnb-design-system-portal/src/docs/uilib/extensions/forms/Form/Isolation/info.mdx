...
## Reset data after commit (`resetAfterCommit`)
...
Technically, when you use `requireCommit` or `resetDataAfterCommit`, the `Form.Isolation` will use its "initial" internal data set to crate a reference. This reference is used to either compare if there is a change or to reset the data context after a commit.
...
## Require the user to commit before submitting (`requireCommit`)

In scenarios where you want to ensure users commit their changes in an isolated section before submitting the entire form, you can use the `requireCommit` property. This will prevent form submission and prompt the user to commit any uncommitted changes first.
