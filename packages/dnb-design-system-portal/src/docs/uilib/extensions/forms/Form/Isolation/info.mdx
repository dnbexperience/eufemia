...
## Reset data after commit

You can reset the isolation the user committed by using `resetAfterCommit`:
...
function MyForm() {
  return (
    <Form.Handler>
      <Form.Isolation resetAfterCommit>
        <Field.String path="/isolated" />
        <Form.Isolation.CommitButton />
      </Form.Isolation>
    </Form.Handler>
...
Technically, when you use `resetAfterCommit`, the `Form.Isolation` will reset its data context based on the "initial" internal data set.

But in some situations, you may need a different data set than the internal data set given at the initial render.

In order to do that you can create a `resetSnapshot` and pass it to the `Form.Isolation` component and call `refresh` on it whenever you need to update it.
...
function MyForm() {
  ...
  return (
    <Form.Handler>
      <Form.Isolation resetAfterCommit resetSnapshot={resetSnapshot}>
        <Field.String path="/isolated" />
        <Form.Isolation.CommitButton />
      </Form.Isolation>
    </Form.Handler>
