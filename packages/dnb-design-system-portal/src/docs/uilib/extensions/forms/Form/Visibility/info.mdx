...
You can disable children fields when they are hidden by using the `fieldPropsWhenHidden` property. It will pass the given props to the children when the visibility is hidden. It needs to be used in combination with the `keepInDOM` property.
