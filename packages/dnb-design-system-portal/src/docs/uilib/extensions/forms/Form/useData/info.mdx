...
The `data` property is the value from the current render. Use `getValue` inside callbacks when you need the latest value without subscribing. The component can still re-render for other reasons, such as parent renders or other context changes.
