...
If you need the same helper API without subscribing the component to form data changes, use `Form.useDataWithoutSubscription`.
...
For custom fields and other internal integrations that only need imperative access to the same helpers, use `Form.useDataWithoutSubscription`. It returns the same API and accepts the same optional `id` and `initialData` arguments as `Form.useData`, but does not subscribe the component to data updates.
...
The `data` property is the value from the current render. Because `Form.useDataWithoutSubscription` does not subscribe to form data updates, `data` can be out of date after the form changes. Use `getValue` inside callbacks when you need the latest value without subscribing. The component can still re-render for other reasons, such as parent renders or other context changes.
