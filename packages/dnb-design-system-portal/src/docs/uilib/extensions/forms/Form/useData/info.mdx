...
It returns `getValue`, `update`, `remove`, `set`, `filterData` and `reduceToVisibleFields`. It does not return `data`, because `data` would be a stale render snapshot after form changes. Use `getValue` when you need the latest value without subscribing.
...
function MyComponent() {
  const { getValue, update, filterData } =
