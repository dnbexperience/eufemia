...
- `reduceToVisibleFields` will use the provided data and remove data entries of hidden (unmounted) fields.
...
You can use the `reduceToVisibleFields` method to get only the data of visible (mounted) fields. Check out the [example](/uilib/extensions/forms/Form/Handler/demos/#visible-data) in the demo section.
