...
When calling the `createSnapshot` or `revertSnapshot` functions, you can pass inn your snapshot the `name` (my-snapshot-slice-name) as the second parameter. This will make sure that the snapshot is only applied to the given part of the form data.
