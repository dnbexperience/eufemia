...
**Note:** The hook must be called with an `id` that matches a [Form.Handler](/uilib/extensions/forms/Form/Handler/) on the page. It throws if no handler with that `id` is found.

## API

- `submit(): Promise<EventStateObject | undefined>` – Triggers form submit (validation + `onSubmit`). Returns a Promise that resolves with the submit result or `undefined`.
