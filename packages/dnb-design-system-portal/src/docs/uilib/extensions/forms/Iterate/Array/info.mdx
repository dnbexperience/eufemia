...
You can also set `minimumContainerItems` to 1, to ensure that the first item does not have a remove button, as long as there is at least one item.

You can also set `hideContainerToolbarWhen`, like `(index, items, mode) => items.length === 1` to hide the toolbar when there is only one item.
