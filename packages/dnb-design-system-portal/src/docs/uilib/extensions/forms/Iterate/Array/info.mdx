...
If you want to be able to provide values to the individual field component directly instead of pointing to them with paths, you can give `Iterate.Array` a render property. It works a bit like an array-map call. The render function receives the value of the item as the first argument, and the index of which item you are on as the second.
