---
showTabs: true
hideInMenu: true
---

import * as Examples from './Examples'

## Demos

### Require commit

This example uses the `preventUncommitedChanges` property to display an error message if the user has made changes and attempts to navigate to the next Wizard step.

Try entering something in the input field, then navigate to the next step. An error message will appear to indicate that changes must be committed first.

<Examples.preventUncommitedChangesText />

### Initially open

<Examples.InitiallyOpen />

### With existing data

<Examples.ViewAndEditContainer />

### Isolated data

This demo shows how to use the `isolatedData` property to provide data to the PushContainer.

<Examples.IsolatedData />
