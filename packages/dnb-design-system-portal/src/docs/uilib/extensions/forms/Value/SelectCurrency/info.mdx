...
You can use the `Value.SelectCurrency.useCurrency` hook to get the currency name by ISO code ([ISO 4217 code](https://en.wikipedia.org/wiki/ISO_4217)). It returns the currency name in the current locale.

```tsx
import { Value } from '@dnb/eufemia/extensions/forms'

const MyComponent = () => {
  const { getCurrencyNameByIso } = Value.SelectCurrency.useCurrency('NOK')
}
```
