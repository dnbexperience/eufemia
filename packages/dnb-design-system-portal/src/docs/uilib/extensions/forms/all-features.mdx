...
`FieldBlock` is a wrapper component for grouping fields together. It can be used to create a layout for fields, and it can also be used to create a field that consists of several subfields, so called [composition fields](/uilib/extensions/forms/base-fields/Composition/).
