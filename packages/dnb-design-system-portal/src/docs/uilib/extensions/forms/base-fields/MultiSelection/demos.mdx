---
showTabs: true
---

import * as Examples from './Examples'

## Demos

### Basic

<Examples.Basic />

### With Select all

This example uses field width `medium`.

<Examples.WithSelectAll />

### With search

<Examples.WithSearch />

### With confirm and cancel

<Examples.WithConfirmCancel />

### With nested items

<Examples.WithNestedItems />

### With selected item tags

<Examples.WithSelectedItems />

### With many selected items

When `showSelectedTags` is enabled and the number of items exceeds the `selectedItemsCollapsibleThreshold` (default: `10`), an accordion appears to collapse/expand the selected items. A "Clear all" button also appears to the right for quickly deselecting all items.

<Examples.WithManySelectedItems />

### With disabled items

<Examples.WithDisabledItems />

### With search config

Use the `search` prop to configure search behavior. This example uses `matchNumbers: true` so account numbers can be searched by typing digits.

<Examples.WithSearchConfig />

### Inline variant

The `inline` variant renders the item list inline without a popover. This is useful when you want the options to be always visible.

<Examples.VariantInline />

### Using dataPath from Form context

<Examples.WithDataPath />

### Disabled field

<Examples.Disabled />

### With minimum required items

<Examples.WithError />
