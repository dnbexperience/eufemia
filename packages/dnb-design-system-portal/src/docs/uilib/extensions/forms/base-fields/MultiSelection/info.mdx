...
### Scrollable Inline Item List

Use `maxHeight` with the `inline` variant to constrain only the options list. The selected tags and search field remain visible while the options scroll. The property accepts any CSS `max-height` value, such as `maxHeight="28rem"`. Numbers are interpreted as rem.
