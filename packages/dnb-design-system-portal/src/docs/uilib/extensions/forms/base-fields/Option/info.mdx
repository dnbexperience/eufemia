...
`Field.Option` is a pseudo-component that is used by parent components to configure options. It has no use on its own. How it renders, and what extra properties it accepts, are defined by the parent.
