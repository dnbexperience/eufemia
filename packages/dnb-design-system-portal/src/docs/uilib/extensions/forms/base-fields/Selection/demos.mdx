---
showTabs: true
---

import * as Examples from './Examples'

## Demos

### Variants summary

As there are many variants, they are split into separate sections. Here is a summary of the variants:

#### Dropdown

<Examples.DropdownLabel />

#### Autocomplete

<Examples.AutocompleteLabel />

#### Radio buttons

<Examples.RadioLabel />

#### Toggle buttons

<Examples.ButtonLabel />

---

### Dropdown variant (default)

#### Dropdown empty

<Examples.DropdownEmpty />

#### Dropdown placeholder

<Examples.DropdownPlaceholder />

#### Dropdown with a transformed selection text

<Examples.DropdownTransformSelection />

#### Dropdown label and option selected

<Examples.DropdownLabelAndOptionSelected />

#### Dropdown with help

<Examples.WithHelp />

### Horizontal layout

<Examples.HorizontalLayout />

#### Dropdown disabled

<Examples.DropdownDisabled />

#### Dropdown option disabled

<Examples.DropdownDisabledOptions />

#### Dropdown error

<Examples.DropdownError />

#### Dropdown dynamic options

<Examples.DropdownDynamicOptions />

#### Dropdown high number of options

<Examples.DropdownHighNumberOfOptions />

#### Dropdown validation - Required

<Examples.DropdownValidationRequired />

#### Dropdown button with a path to populate the data

<Examples.DropdownWithAPath />

#### Dropdown with the data property

<Examples.DropdownWithData />

### Dropdown widths

<Examples.DropdownWidths />

### Dropdown groups

<Examples.DropdownGroups />

---

### Autocomplete variant

<Examples.AutocompleteValidationRequired />

### Autocomplete groups

<Examples.AutocompleteGroups />

---

### Radio variant

#### Radio empty

<Examples.RadioEmpty />

#### Radio option selected

<Examples.RadioOptionSelected />

#### Radio horizontal layout

<Examples.RadioHorizontalLayout />

#### Radio horizontal options-layout

<Examples.RadioHorizontalOptionsLayout />

#### Radio horizontal layout and horizontal options-layout

<Examples.RadioHorizontalLayoutAndHorizontalOptionsLayout />

#### Radio disabled

<Examples.RadioDisabled />

#### Radio option disabled

<Examples.RadioDisabledOptions />

#### Radio error

<Examples.RadioError />

#### Radio button with a path to populate the data

<Examples.RadioWithAPath />

#### Radio with the data property

<Examples.RadioWithData />

#### Radio with List composition

Use render prop children to compose each option with [List](/uilib/components/list) and set selected state from the current field value.

<Examples.RadioWithListComposition />

#### Radio nesting other fields with logic

You can nest other fields and show them based on your desired logic.

<Examples.RadioNestingWithLogic />

#### Radio nesting advanced

<Examples.RadioNestingAdvanced />

---

### Buttons variant

#### ToggleButton empty

<Examples.ButtonEmpty />

#### ToggleButton option selected

<Examples.ButtonOptionSelected />

#### ToggleButton horizontal options-layout

<Examples.ButtonHorizontalOptionsLayout />

#### ToggleButton disabled

<Examples.ButtonDisabled />

#### ToggleButton option disabled

<Examples.ButtonDisabledOptions />

#### ToggleButton error

<Examples.ButtonError />

#### ToggleButton nesting other fields with logic

You can nest other fields and show them based on your desired logic.

<Examples.ButtonNestingWithLogic />

#### ToggleButton with a path to populate the data

<Examples.ButtonWithAPath />

#### ToggleButton with the data property

<Examples.ButtonWithData />

---

### Radio button variant

#### Radio button empty

<Examples.RadioButtonEmpty />

#### Radio button option selected

<Examples.RadioButtonOptionSelected />

#### Radio button horizontal options-layout

<Examples.RadioButtonHorizontalOptionsLayout />

#### Radio button disabled

<Examples.RadioButtonDisabled />

#### Radio button error

<Examples.RadioButtonError />

#### Radio button with the data property

<Examples.RadioButtonWithData />

<VisibleWhenVisualTest>
  <Examples.SelectionErrorMessage />
  <Examples.SelectionInfoMessage />
  <Examples.SelectionWarningMessage />
</VisibleWhenVisualTest>
