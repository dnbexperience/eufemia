...
Multiline fields show a vertical resize handle by default, including when `autoResize` is enabled. Use `hideResizeHandle` to prevent users from manually resizing the field.
