...
| `submitElement`                         | `React.Element`       | _(optional)_ accepts a React element which will show up like the "submit button" would do on `type="search"`. Won't work if `multiline` is `true` or `mask` is used, as `Textarea` and `InputMasked` dont support this prop. |
