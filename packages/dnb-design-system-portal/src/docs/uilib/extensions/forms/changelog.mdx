...
- Added `preventUncommitedChanges` property to [Form.Isolation](/uilib/extensions/forms/Form/Isolation/) to show error during submit after user input.
- Renamed `requireCommit` to `preventUncommitedChanges` in [Iterate.PushContainer](/uilib/extensions/forms/Iterate/PushContainer/).
