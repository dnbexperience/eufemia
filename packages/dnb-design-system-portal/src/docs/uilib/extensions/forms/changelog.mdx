...
- [Field.Currency](/uilib/extensions/forms/feature-fields/Currency/) defaults to `disallowLeadingZeroes`.
- Fix handling of invalid ISO code value in [Value.SelectCountry](/uilib/extensions/forms/Value/SelectCountry/).
- Fix handling of invalid ISO code value in [Value.SelectCurrency](/uilib/extensions/forms/Value/SelectCurrency/).
- Fix so [Value.BankAccountNumber](/uilib/extensions/forms/Value/BankAccountNumber/) does not render when value is empty.
- Fix so [Value.OrganizationNumber](/uilib/extensions/forms/Value/OrganizationNumber/) does not render when value is empty.
- Fix so [Value.PhoneNumber](/uilib/extensions/forms/Value/PhoneNumber/) does not render when value is empty.
- Fix so [Value.NationalIdentityNumber](/uilib/extensions/forms/Value/NationalIdentityNumber/) does not render when value is empty.
- Fix so [Wizard.Step](/uilib/extensions/forms/Wizard/Step/) does not pre-render step when `keepInDOM`.
...
- Fix anchor alignment in the redesigned [Wizard](/uilib/extensions/forms/Wizard/).
- Fix font size of items in the redesigned [Wizard](/uilib/extensions/forms/Wizard/).
...
- Deprecated `sidebarId` in [Wizard.Container](/uilib/extensions/forms/Wizard/Container/), as it no longer has any sidebar. If an id is needed, use the `id` prop.
...
- Fixed so [Value.Composition](/uilib/extensions/forms/Value/Composition/) inherits props defined in [Value.Provider](/uilib/extensions/forms/Value/Provider/).
...
- Fixed so `aria-required` and `required` props is set in the `input` element for [Field.Upload](/uilib/extensions/forms/feature-fields/more-fields/Upload/).
