...
- [Field.PhoneNumber](/uilib/extensions/forms/feature-fields/PhoneNumber/) now emits values in pure E.164 format without spaces (e.g. `+4712345678` instead of `+47 12345678`). Patterns and schemas must be updated to match.
- Added auto-detection of all E.164 country codes from values without a space separator in [Field.PhoneNumber](/uilib/extensions/forms/feature-fields/PhoneNumber/) and [Value.PhoneNumber](/uilib/extensions/forms/Value/PhoneNumber/).
- Added support for `00` international dialing prefix (e.g. `004712345678`) — automatically interpreted as `+` in [Field.PhoneNumber](/uilib/extensions/forms/feature-fields/PhoneNumber/), [Value.PhoneNumber](/uilib/extensions/forms/Value/PhoneNumber/), and the `detectCountryCode` utility.
