...
For creating a `Value.*` component you can use [ValueBlock](/uilib/extensions/forms/create-component/ValueBlock/):
...
In certain situations, you may find the [useFieldProps](/uilib/extensions/forms/create-component/useFieldProps/) hook useful (despite its name, useFieldProps, it's also applicable to value components). This hook offers convenient data transformation methods that can be readily exposed when wrapped (yes it's named `useFieldProps`, but it's also used for value components).
