...
- `dataContext` the internal shared data context. It is intended for low-level integrations; read render-dependent values through `value`, `Form.useData().getValue('/path')` or validator connection helpers instead of reading `dataContext.data` during render. Use `Form.useDataWithoutSubscription().getValue('/path')` only for non-subscribing imperative reads in callbacks, effects or internal handlers.
