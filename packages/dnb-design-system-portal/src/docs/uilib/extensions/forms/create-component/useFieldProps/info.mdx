...
- `withValidatorOptions(validator, { runOnSubmit: 'never' })` keeps that validator from running during form submit. The default is `runOnSubmit: 'always'`. Use `runOnSubmit: 'when-changed'` to run during submit only if the value changed since the validator last ran.
