...
- `dataContext` the internal shared data context. It is intended for low-level integrations; read values through `value`, `Form.useData().getValue('/path')` or validator connection helpers instead of reading `dataContext.data` during render.
