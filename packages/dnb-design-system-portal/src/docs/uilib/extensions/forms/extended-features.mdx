...
You can provide [custom logic](/uilib/extensions/forms/getting-started/#validation-and-error-handling) and messages to handle and display error message. More details about error message can be found [on a separate page](/uilib/extensions/forms/extended-features/Form/error-messages).
