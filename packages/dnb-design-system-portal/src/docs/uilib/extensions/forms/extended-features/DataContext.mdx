...
In the case of forms divided into several steps, the combination of the components `DataContext.Provider` and `Steps.Steps` will also ensure that only the fields that are visible on the screen (for the relevant step, or based on what is hidden or shown via the Visibility component) provide a basis for whether one can proceed in the process or not.
