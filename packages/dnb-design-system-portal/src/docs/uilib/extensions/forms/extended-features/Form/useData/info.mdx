...
function Component() {
  const { data } = Form.useData('unique', initialData)
...
function Component() {
  const { update } = Form.useData('unique', (data = { initialData }))
...
With the `set` method, you can replace the whole data set if needed.
