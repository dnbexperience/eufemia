...
function MyForm() {
  ...
  return (
    <Form.Handler>
      <Wizard.Layout initialActiveIndex={getIndex()} id="unique-id">
        ...
      </Wizard.Layout>
    </Form.Handler>
  )
...
You connect the hook with the `Wizard.Layout` component via an unique `id` (string). The `id` will be used in the URL query string: `url?unique-id-step=1`.
