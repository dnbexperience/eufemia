---
showTabs: true
---

## Description

`Wizard.useStep` is a React Hook that returns `Wizard.Context` parameters such as totalSteps, activeIndex or a setActiveIndex handler.

```jsx
import { Form, Wizard } from '@dnb/eufemia/extensions/forms'

function Step1() {
  const { totalSteps, activeIndex, setActiveIndex } = Wizard.useStep()

  return <Wizard.Step>...</Wizard.Step>
}

function MyForm() {
  return (
    <Form.Handler>
      <Wizard.Container>
        <Step1 />
      </Wizard.Container>
    </Form.Handler>
  )
}
```

You can also connect the hook with the `Wizard.Container` via an `id` (string). This lets you render the hook outside of the context:

```jsx
import { Form } from '@dnb/eufemia/extensions/forms'

function Sidecar() {
  const { activeIndex, setActiveIndex } = Wizard.useStep('unique-id')
}

function MyForm() {
  return (
    <Form.Handler>
      <Sidecar />
      <Wizard.Container id="unique-id">...</Wizard.Container>
    </Form.Handler>
  )
}
```

## EditButton

In order to navigate to a new step when using `setActiveIndex` you can use the [EditButton](/uilib/extensions/forms/extended-features/Wizard/Step/EditButton/) component.
