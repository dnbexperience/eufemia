...
### useMinimumAge and createMinimumAgeChecker

To use the `createMinimumAgeValidator` functionality without a field, you can use `useMinimumAge` or `createMinimumAgeChecker`, which returns a boolean.

```tsx
import {
  useMinimumAge,
  createMinimumAgeChecker,
} from '@dnb/eufemia/extensions/forms/Field/NationalIdentityNumber'
...
function Component() {
  const isAdult = useMinimumAge(18)
