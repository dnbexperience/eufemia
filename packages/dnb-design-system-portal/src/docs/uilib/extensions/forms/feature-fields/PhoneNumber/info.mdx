...
Some backends may store phone numbers with a `00` prefix instead of `+` (e.g. `004712345678`). Since the component uses E.164 format (`+` prefix) internally, normalize the value before passing it to the component and use `transformOut` to convert back on output:

```tsx
const backendValue = '004712345678'

<Field.PhoneNumber
  value={backendValue.replace(/^00/, '+')}
  omitSpaceSeparator
  transformOut={(value) =>
