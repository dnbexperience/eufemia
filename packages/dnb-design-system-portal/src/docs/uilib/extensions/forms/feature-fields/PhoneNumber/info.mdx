...
Some backends may store phone numbers with a `00` prefix instead of `+` (e.g. `004712345678`). Since the component uses E.164 format (`+` prefix) internally, you can use `transformIn` and `transformOut` to convert between formats:
...
Note that the `value` prop should use E.164 format (`+` prefix). Normalize the value before passing it to the component if your backend stores it with a `00` prefix.
