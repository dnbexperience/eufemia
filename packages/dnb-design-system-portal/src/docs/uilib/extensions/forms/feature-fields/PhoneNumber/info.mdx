...
### Storing values without space separator

By default, the component emits values with a space between the country code and the phone number (e.g. `+47 12345678`). If your backend requires values without the space (e.g. `+4712345678`), use the `omitSpaceSeparator` prop:
...
This ensures the value round-trips correctly: spaceless input from the backend is auto-detected on the way in, and is emitted without a space on the way out.

When combining `omitSpaceSeparator` with `transformOut`, note that the value passed to `transformOut` is already in E.164 format (e.g. `"+4712345678"`), so your transformer receives the spaceless string directly.
...
Other international dialing prefixes such as `011` (US/Canada), `0011` (Australia), or `001` (Thailand, Singapore) are **not** supported, because they create ambiguous interpretations. For example, `0014712345678` could be parsed as either `00` + `1` (US) + `4712345678` or `001` (Thai IDD) + `47` (Norway) + `12345678` — two entirely different phone numbers. The `00` prefix does not have this problem, since no E.164 country code starts with `0`. For systems that use other prefixes, use `transformIn` to normalize the value before it reaches the component.
