...
When a phone number value is provided without a space separator between the country code and the number (e.g. `+4712345678`), the component will auto-detect the Norwegian `+47` country code and split the value correctly. This is useful when receiving phone numbers from a backend system that does not include a space separator.
