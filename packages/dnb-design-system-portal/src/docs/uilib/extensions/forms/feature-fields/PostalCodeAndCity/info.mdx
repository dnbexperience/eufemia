...
For countries that use a four-digit postal code (`NO`, `DK` and `CH`), the placeholder code `0000` is rejected as invalid. This built-in check is applied automatically.

If you provide your own `postalCode.onChangeValidator`, it replaces the built-in check. You can still keep it by composing the exported `postalCodeValidator`, which is made available through the second argument of your validator:
