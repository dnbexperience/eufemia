...
For countries that use a four-digit postal code (`NO`, `DK` and `CH`), the placeholder code `0000` is rejected as invalid. This built-in check is applied automatically when no `onChangeValidator` is provided for the postal code field.

If you provide your own `postalCode.onChangeValidator`, it replaces the built-in check. You can still reuse the built-in rule by importing `postalCodeValidator` and composing it with your own validator:

```tsx
import { Field, postalCodeValidator } from '@dnb/eufemia/extensions/forms'
  ...
  <Field.PostalCodeAndCity
    postalCode={{
      onChangeValidator: (value) =>
        ...
        (value === '1111'
          ? new Error('This postal code is not allowed.')
          : undefined),
