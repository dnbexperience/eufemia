...
It supports the HTML [autocomplete](https://developer.mozilla.org/en-US/docs/Web/HTML/Attributes/autocomplete) attribute, and by default set it to `current-password`. Consider setting value to `new-password` when it's expected that the user should enter a new password.
