...
The returned data is an array of objects containing a file object, an unique ID, etc. The [file item object](/uilib/components/upload/properties/#fileitem) contains the file itself and some additional properties like an unique ID.
...
If doing so, it's recommended to pass along the rest of the [file item object](/uilib/components/upload/properties/#fileitem) using the spread operator or so (...fileItemObj), as it can contain properties needed internally that one is not aware of, or updated values since last file was uploaded, or even future new internal properties that does not exist yet.
