...
You can filter data by any given criteria. This is done by adding a `filterData` property with a handler to the `Form.Handler`.
...
For monitoring your form for errors, you can use the [Form.useError](/uilib/extensions/forms/extended-features/Form/useError) hook.
...
Async validation is also supported. The validator function can return a promise that resolves to an error message, which is shown below the field:

```tsx
const validator = (value) => {
  return new Promise((resolve, reject) => {
    // Delay the response
    resolve(new Error('Invalid value')) // Show this message below the field
  })
}
render(<Field.PhoneNumber validator={validator} />)
```
...
In this example, we use the `async` and `await` keywords to make the code more readable. Also, we use `onBlurValidator` to only validate the field when the user leaves the field:
...
When using async validation, it's a good idea to debounce the validation function to avoid unnecessary requests. This can be done by using the [debounceAsync](/uilib/helpers/functions/#debounce) helper function:
