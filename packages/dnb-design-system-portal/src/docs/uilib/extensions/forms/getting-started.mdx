...
```tsx
import { withValidatorOptions } from '@dnb/eufemia/extensions/forms'
...
Use `runOnSubmit: 'when-changed'` when the validator should run during submit only if the field value changed since the validator last ran.
