...
To build an entire form, there are surrounding components such as form [Handler](/uilib/extensions/forms/extended-features/Form/Handler) and [Wizard.Layout](/uilib/extensions/forms/extended-features/Wizard/Layout) that make data flow and layout easier and save you a lot of extra code, without compromising flexibility.
...
When using [Wizard.Layout](/uilib/extensions/forms/extended-features/Wizard/Layout/) you can use in addition:

```tsx
<Wizard.Layout onStepChange={async () => {}}>...</Wizard.Layout>
```

It will disable all fields and show an indicator on the [NextButton](/uilib/extensions/forms/extended-features/Wizard/NextButton/) while the **step** is pending ([examples](/uilib/extensions/forms/extended-features/Wizard/Layout/demos/)).
