...
By default, the country code is set to `NO`.

Changing it will ensure that e.g. the [PostalCodeAndCity](/uilib/extensions/forms/feature-fields/PostalCodeAndCity/) field will disable validation when the country code is not `NO`.
