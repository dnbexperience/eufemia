...
When the user makes a value change, it will show an indicator for the corresponding field while the change is pending.
...
The suffix is displayed in a secondary text color to distinguish it from the label while keeping it inside the label.
...
#### Optional and supporting label text in designs

When implementing a screenshot or Figma design, identify the role of text around the field before mapping it to a property. In a form where fields are required, use `required={false}` when a label marks one field as optional. Use `labelDescription` for separate supporting information, and add `labelDescriptionInline` when the supporting text appears on the same line as the label. These properties can be used together.

Do not render "optional" or "valgfritt" with the classic Input `suffix`. An input suffix is content displayed after the input, such as contextual help, a unit or currency, rather than label metadata.
