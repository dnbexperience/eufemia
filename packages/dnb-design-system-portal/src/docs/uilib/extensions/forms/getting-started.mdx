...
For filtering data during form submit (`onSubmit`), you can either use the `filterSubmitData` property or the `filterData` method given as a parameter to the `onSubmit` event callback:
