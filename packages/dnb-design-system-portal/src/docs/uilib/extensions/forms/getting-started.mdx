...
The `optional` property adds "(optional)" to the field label and ensures the field is not required. Even if all fields in the form are made required by inheriting the required property from a component, like [Form.Handler](/uilib/extensions/forms/Form/Handler/), the optional field will still not be required.

```tsx
<Form.Handler required>
  <Field.String label="I'm required" />
  <Field.String label="I'm not" optional />
</Form.Handler>
```
