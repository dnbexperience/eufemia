...
The `schema` property is a schema that the value of the field must match. **Zod schemas are preferred** as they don't require AJV:

**Preferred: Using Zod schemas (no AJV required)**
...
**Alternative: Using JSON Schema (requires AJV)**

```tsx
import { Form, Field, Ajv } from '@dnb/eufemia/extensions/forms'
