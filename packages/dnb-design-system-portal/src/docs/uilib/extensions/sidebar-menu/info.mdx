...
- A notification badge on a nested item is summarized by a red dot on ancestor accordions. The dot remains visible when the accordion expands and takes priority over an informational badge on the same menu point. An explicit notification badge remains visible instead of the dot.
