...
When `onCollapse` is provided, continued pointer dragging below `collapseThreshold` calls it. The threshold defaults to half of `minWidth`. Pair it with `useResponsive().collapseInline()` to dismiss desktop inline navigation and let `ResponsiveTrigger` restore it.
