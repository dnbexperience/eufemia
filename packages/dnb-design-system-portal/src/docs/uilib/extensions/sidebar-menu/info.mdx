...
- `ResponsiveInline` renders its children above `drawerAt`. Between `drawerAt` and the optional `compactAt` threshold, its SidebarMenu becomes a narrow icon rail that can expand over adjacent content on hover or keyboard focus.
...
Set `compactAt` when the inline menu should reduce to an icon rail before it becomes a Drawer. `compactOffset` adds an additional viewport width to that threshold, so `compactAt="large" compactOffset="10em"` becomes compact at `82em`. Set `expandedWidth` and `compactWidth` on `ResponsiveAside`; it reserves only the compact width and expands the full menu over adjacent content on hover or keyboard focus. Compact mode is disabled on touch-only input so the first touch still activates an item. Every interactive menu row should have an icon in compact mode. Use `ResponsiveInline` directly only when the application needs to build a different sidebar surface.
