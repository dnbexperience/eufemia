...
Set `dividerBefore: true` on a data item to render a separator, or place `SidebarMenu.Divider` between declarative items. Set `openOnFind={false}` on the container when collapsed content should not use `hidden="until-found"`.
...
Before hydration, the default `medium` breakpoint controls the CSS visibility needed for stable server-rendered markup. A custom `breakpoint` takes over after hydration. Add the matching application CSS when a custom breakpoint must also apply before JavaScript runs.
