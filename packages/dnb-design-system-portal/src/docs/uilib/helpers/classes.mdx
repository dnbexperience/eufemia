...
`dnb-compact-drop-shadow`

Adds a smaller, but sharper drop shadow, `box-shadow: 0 1px 6px rgba(0, 0, 0, 0.16)`, to the component. Designed for hovering boxes such a dropdown or calendar.

### CSS properties

The DNB Compact drop shadow is also available as a CSS Custom Property:

```ts
import properties from '@dnb/eufemia/style/themes/theme-ui/properties.js'

const cssBoxShadow = properties['--shadow-compact']
```

If you only want to apply parts of the property, these are available as well:

- `--shadow-compact-x: 0;`
- `--shadow-compact-y: 1px;`
- `--shadow-compact-blur-radius: 6px;`
- `--shadow-compact-color: rgba(0, 0, 0, 0.16);`
