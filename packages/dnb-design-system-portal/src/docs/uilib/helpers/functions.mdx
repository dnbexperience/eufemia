...
Prevents React crashes caused by Google Translate (and similar extensions) mutating the DOM.

Google Translate wraps text nodes in `<font>` elements, moving them out of their original parent. When React later tries to remove or insert before those nodes, it throws because the parent reference no longer matches. This patch makes `removeChild` and `insertBefore` find and operate on the `<font>` wrapper instead.

The patches are deferred — a lightweight observer watches `<html>` for the `translated-*` class that Google Translate adds before it starts mutating text nodes. Call the function once at the top level of your application:
...
Calling it more than once is safe — subsequent calls are no-ops. The patch is permanent for the lifetime of the page.

See the [React issue](https://github.com/facebook/react/issues/11538) and [Everything about Google Translate crashing React](https://martijnhols.nl/blog/everything-about-google-translate-crashing-react) for background.

#### Demo

import { GoogleTranslateDemo } from './Examples'
