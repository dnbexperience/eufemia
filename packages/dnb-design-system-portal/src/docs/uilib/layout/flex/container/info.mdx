...
Ideally, use [Flex.Item](/uilib/layout/flex/item) or [Card](/uilib/components/card) for you inner wrappers.
...
During render, the items within the "Wrapper" container are wrapped with the same properties. This ensures that all the items have a same appearance.
