...
For doing so, provide a `span` property with an object containing [Media Query](/uilib/layout/media-queries/) types. Each media span should contain number, like mentioned above.
