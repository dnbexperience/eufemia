...
Use `gapBefore` or `gapAfter` to replace the container gap next to a specific item. They follow the container's main axis: before/after maps to left/right in horizontal layouts and top/bottom in vertical layouts. Use `false` to remove that gap.
