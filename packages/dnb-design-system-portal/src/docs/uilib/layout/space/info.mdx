...
Use `defaultBreakpoint` to set which breakpoint's spacing scale to use as the default. It defaults to `medium` (the basis scale). Set `small` to default to compact spacing, or `large` to default to spacious spacing.

```tsx
<Space.Responsive defaultBreakpoint="small">
  <MyComponent top="large" />
</Space.Responsive>
```

#### Density

Use `density` to force a specific spacing density regardless of viewport size and `defaultBreakpoint`. When set, it overrides the default breakpoint scale.
...
| Prop                | Description                                                                        |
| ------------------- | ---------------------------------------------------------------------------------- |
| `defaultBreakpoint` | Which breakpoint's scale to default to (`small`, `medium`, `large`).               |
| `density`           | Forces a specific density, overriding `defaultBreakpoint` (`compact`, `spacious`). |
