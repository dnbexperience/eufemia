...
import {
  ...
  ResponsiveTypographySbankenExample,
...
**NB**: Responsive typography is turned on by default for Sbanken brand.

Use `Typography.Context responsive` to make typography components, or all components use responsive font size and line heights.

To turn off responsive typography for a section you can use `Typography.Context responsive={false}`
