...
All `--responsive-font-size-*` CSS variables are scoped to typography components as of now. If you need to use them elsewhere use the `.dnb-t` class.

#### Turning all typography responsive (BETA)

If you use `<Typography.Context responsiveAll>` all the regular font size variables `--font-size-*` will be remapped to the responsive `--responsive-font-size-*` values.
