...
All `--responsive-line-height-*` CSS variables are scoped to typography components as of now. If you need to use them elsewhere you can use the `.dnb-t` class.

#### Turning all typography responsive (BETA)

If you use `<Typography.Context responsiveAll>` all the regular line-height variables `--line-height-*` will be remapped to the responsive `--responsive-line-height-*` values.

It can be turned off using `<Typography.Context responsiveAll={false}>`

Since `responsiveAll={true}` remaps the global variables, it will effectively override `responsive={false}`
