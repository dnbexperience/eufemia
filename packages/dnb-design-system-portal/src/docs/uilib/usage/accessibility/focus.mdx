...
A more complex focus management is already built into the [Modal Component](/uilib/components/modal). The Modal actually disables focus possibility on/of the content behind the Modal, so the user only can navigate inside the modal.
