...
Design tokens are included when you import an Eufemia theme. No separate import is needed.
...
- **Eiendom** currently reuses the UI token file, so the three explicit brand references are **DNB**, **Sbanken** and **Carnegie**.
