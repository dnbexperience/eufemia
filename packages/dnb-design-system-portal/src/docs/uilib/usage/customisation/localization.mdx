...
render(<Provider translations={svSE}>Your app</Provider>)
```

### Eufemia Forms

For easily merging translations, you can use the `mergeTranslations` function from `@dnb/eufemia/shared`.
...
Or set the `translations` property within the [Form.Handler](/uilib/extensions/forms/Form/Handler/) component like this:
