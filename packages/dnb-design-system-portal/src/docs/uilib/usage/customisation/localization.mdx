...
ICU support is opt-in to keep your bundle size small. Enable it by importing the `icu` handler and passing it to the `Provider`:

```tsx
import { icu, Provider } from '@dnb/eufemia/shared'

render(
  <Provider icu={icu} locale="en-GB">
    <App />
  </Provider>
)
```
...
render(
  <Provider icu={icu} translations={translations} locale="en-GB">
    <NotificationBanner />
  </Provider>
)
```
