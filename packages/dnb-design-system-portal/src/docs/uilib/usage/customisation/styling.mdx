...
- **dnb-ui-core**: includes the _DNB Main Styles_ like [helper classes](/uilib/helpers), HTML and body reset (normalize).
  - We have an alternative package, **dnb-ui-basis**, that can be used if your page has other css that conflicts with Eufemia. It does the same,
    but adds resets to the `.dnb-core-style` class instead of the `body` element so Eufemia styling can be scoped to just a part of your page.
...
Tailwind can be used as a tooling layer alongside Eufemia, but avoid overlapping CSS and resets.

**Recommended approach:**

- Use Eufemia as the source of truth for tokens and reset.
- Do not include Tailwind's base/reset; only include its `theme` + `utilities`.
- Map Tailwind tokens to Eufemia CSS variables; avoid self-referencing variables.

NB: import only Tailwind theme and utilities (Tailwind v4):
...
Example: bridge Tailwind tokens to Eufemia variables using `@theme`:
...
@theme {
  ...
  /* typography */
  --font-family-sans: var(--font-family-default);
  --font-family-mono: var(--font-family-monospace);
}
```
