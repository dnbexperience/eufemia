...
- **dnb-theme-basis**: includes [optional class selectors for elements](/uilib/elements), [optional default spacing](#spacing), [default typography](/uilib/typography).
- **dnb-theme-components**: includes all the styles for [components](/uilib/components).

### Additional Theme Packages

- **dnb-theme-properties**: includes only the CSS Custom Properties. You find this package is inside `/style/themes/theme-ui/`.
...
- **dnb-theme-extensions**: includes all the styles (and themes) for [extensions](/uilib/extensions) (not shown in the [Diagram](#css-structure-diagram)).
- **dnb-theme-fonts**: includes only the `@font-face` and properties. You find this package is inside `/style/themes/theme-ui/`.
- **dnb-theme-tags**: this package will force styles on all the HTML Tags like `<h1>` instead of CSS classes, like `.dnb-h--xx-large`
  - **NB:** needs a `.dnb-core-style` [wrapper class](/uilib/usage/customisation/styling#core-style).
...
The styles for the `.dnb-spacing` are included in the package: **dnb-theme-basis**
...
import '@dnb/eufemia/style/themes/theme-ui/dnb-theme-tags.min.css'
