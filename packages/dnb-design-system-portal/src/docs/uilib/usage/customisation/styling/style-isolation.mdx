...
- Font files are loaded from the [immutable CDN path matching the Eufemia version](/uilib/usage/assets/#stable-and-versioned-urls), so isolated applications do not silently change when a newer release updates a font.
