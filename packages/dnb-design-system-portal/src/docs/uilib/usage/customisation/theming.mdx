...
showTabs: true
redirect_from:
  - /brand/development/import-styles
tabs:
  - title: Overview
...
### Machine-readable theme capabilities

Tools and applications can inspect the same support matrix used by Eufemia's
types and documentation:

| Theme      | Color schemes   | Dark mode stylesheet                                                | Dark surfaces |
| ---------- | --------------- | ------------------------------------------------------------------- | ------------- |
| `ui`       | `light`, `dark` | `@dnb/eufemia/style/themes/ui/ui-theme-dark-mode.min.css`           | Yes           |
| `sbanken`  | `light`, `dark` | `@dnb/eufemia/style/themes/sbanken/sbanken-theme-dark-mode.min.css` | Yes           |
| `eiendom`  | `light`, `dark` | `@dnb/eufemia/style/themes/eiendom/eiendom-theme-dark-mode.min.css` | Yes           |
| `carnegie` | `light`         | None                                                                | Yes           |

```tsx
import { themeCapabilities } from '@dnb/eufemia/style/themes/capabilities'

const carnegie = themeCapabilities.carnegie
// { colorSchemes: ['light'], darkModeStyle: null, supportsDarkSurface: true }
```

The metadata describes supported behavior; it does not load theme CSS. Import
the stylesheet named by `darkModeStyle` when enabling a supported dark color
scheme. The table is also available to MCP clients through the packaged
documentation.
