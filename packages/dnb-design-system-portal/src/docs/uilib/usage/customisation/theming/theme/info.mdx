...
These are inline scripts. A policy such as `script-src 'nonce-...'` blocks them unless they carry the matching nonce, which brings the flash back. Pass your per-request nonce to each script:

```tsx
<html>
  <head>
    <ColorSchemeHeadScript nonce={nonce} />
  </head>
  <body>
    <ColorSchemeBodyFirstScript nonce={nonce} />
    {/* your app content */}
    <ColorSchemeBodyLastScript nonce={nonce} />
  </body>
</html>
```

All three accept the standard `<script>` attributes, so `id` and `data-*` can be forwarded the same way. This lets you keep a strict policy instead of allowing `unsafe-inline`.
