...
The MCP server provides documentation context only; it does not execute code.
...
It also provides `portal_content_workflow`, which gives an agent the complete
workflow for editing Portal content and delivering a focused pull request.
...
The local MCP server sends **anonymous, aggregate usage statistics** so we can see which tools, components and documentation areas are most used and improve the docs accordingly. It is **on by default** and **opt-out**.

What is collected is a closed, public vocabulary only: the tool name, the component name for a component lookup (e.g. `Button`, `Field.Address`), the leading documentation area for a doc read or list (e.g. `/uilib/components/`, gated against the areas actually present in the docs — a path outside the docs is dropped, not narrowed), and the `@dnb/eufemia` version you have installed. No machine identifier, user, session or correlation id, and no free text are in that payload — in particular a `docs_search` query is never collected (a search is recorded as a tool count only). The request's source IP is visible to the network layer like any HTTP call, but it is not part of this payload and is not stored on the ingest side.

Each event is a fire-and-forget beacon with a short timeout, so it never blocks or affects a tool call, and the server holds no credentials.

Turn it off by setting `EUFEMIA_MCP_TELEMETRY` to `0` in your MCP client config:
...
### Why use skills with MCP?

The MCP server can provide the same workflows as tools and current
documentation. Skills complement it by helping an agent recognize relevant
requests and choose the right Eufemia tools without the user naming them. They
also make the workflows available in clients that do not support MCP. Keep
skills focused on discovery and reusable workflow instructions, and retrieve
version-sensitive facts from the MCP server.
...
After upgrading `@dnb/eufemia`, update the installed project skills from the
...
The command opens the same selector with managed locations selected. It updates
the selected skills and their lock file, but refuses to overwrite locally
modified skill files. Review those changes first; use `--force` only when
replacing them intentionally. In automation, pass one location explicitly, for
example `yarn eufemia skills update --target .agents/skills`.

Run this command in the project rather than through the MCP server. The hosted
HTTP server cannot access a caller's project files, and the package-local MCP
server intentionally exposes the same documentation-only tools.
...
`check` uses `.claude/skills` unless you pass another installed location with
`--target`. Without `--target`, `uninstall` removes every managed supported
location in the project. It validates all selected locations before removing
any files. `check` reports missing, changed, stale, or version-mismatched files.
`uninstall` removes only files tracked by Eufemia skills locks and leaves
unrelated project skills untouched.
...
Install the Agent Skills and configure one of the MCP options above. Skills are
...
MCP clients can call `review_rules` to retrieve the same catalog with the
stable rule IDs included in the response. The catalog currently covers the
