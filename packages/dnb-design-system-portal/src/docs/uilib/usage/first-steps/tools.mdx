...
1. **Use the hosted MCP server** at `INSERT_CORRECT_URL/mcp` — no installation needed, always serves the latest released docs.
...
```txt
INSERT_CORRECT_URL/mcp
```

It supports the modern Streamable HTTP transport, and serves the same documentation tools (`docs_entry`, `docs_search`, `component_find`, etc.) as the local server below. A health endpoint is available at `INSERT_CORRECT_URL/healthz`.

#### Example: Claude CLI / raicode CLI

```bash
claude mcp add --transport http eufemia INSERT_CORRECT_URL/mcp
# or
raicode mcp add --transport http eufemia INSERT_CORRECT_URL/mcp
```
