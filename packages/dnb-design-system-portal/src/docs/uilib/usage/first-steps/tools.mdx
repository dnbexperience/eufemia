...
The default destination is `.claude/skills`. Both Claude Code and VS Code
Copilot discover project skills there. To use another supported Agent Skills
location, set it explicitly:

```bash
yarn eufemia skills install --target .github/skills
# or
...
cloud agents. The installer also writes `.eufemia-skills-lock.json` in the
selected skills directory. Commit that file together with the generated skills
so updates can detect local changes.
...
`check` reports missing, changed, stale, or version-mismatched files. `update`
refuses to overwrite locally modified skill files. Review those changes first;
use `--force` only when replacing them intentionally. `uninstall` removes only
files tracked by the Eufemia skills lock and leaves unrelated project skills
untouched.
