...
What is collected is a closed, public vocabulary only: the tool name, the component name for a component lookup (e.g. `Button`, `Field.Address`), the leading documentation area for a doc read or list (e.g. `/uilib/components/`, not the full path), and the `@dnb/eufemia` version you have installed. No machine identifier, user, session or correlation id, and no free text are in that payload — in particular a `docs_search` query is never collected (a search is recorded as a tool count only). The request's source IP is visible to the network layer like any HTTP call, but it is not part of this payload and is not stored on the ingest side.
