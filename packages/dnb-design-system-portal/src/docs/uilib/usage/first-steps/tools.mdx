...
// or for legacy config (ESLint v7 and earlier):
export default [eufemiaEslint.legacyRecommended]
```
...
The first shipped ESLint and Stylelint rule reports deprecated `--color-*` CSS variables. Use design tokens instead.
