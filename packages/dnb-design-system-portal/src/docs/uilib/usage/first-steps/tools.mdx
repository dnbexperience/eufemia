...
And `migration_index`, which answers version questions — when a property,
event, or component was added, deprecated, or removed — so an agent can tell
whether something exists in the version you use, and what replaced it. Narrow
it with a `component` (for example `Button`), a `fromVersion`/`toVersion`
range, and optionally a `changeType`; a call with no filter returns per-version
counts only, because the full index is large. Values marked `sinceInferred` or
`sinceFloor` come from git history rather than an author (`sinceFloor` meaning
"at or before"), so treat them as approximate.
