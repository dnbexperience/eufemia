...
1. **Use the hosted MCP server** at `https://web.eufemia.workers.dev/mcp` — no installation needed, always serves the latest released docs.
...
```txt
https://web.eufemia.workers.dev/mcp
```

It is hosted on Cloudflare Workers, supports the modern Streamable HTTP transport, and serves the same documentation tools (`docs_entry`, `docs_search`, `component_find`, etc.) as the local server below. A health endpoint is available at `https://web.eufemia.workers.dev/healthz`.

#### Example: Claude CLI / raicode CLI

```bash
claude mcp add --transport http eufemia https://web.eufemia.workers.dev/mcp
# or
raicode mcp add --transport http eufemia https://web.eufemia.workers.dev/mcp
```
