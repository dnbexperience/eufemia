...
- The MCP server provides documentation context only; it does not execute code. It sends anonymous, opt-out usage statistics (see [Telemetry](#telemetry) below) but never reads your code, files or search queries.
...
What is collected is a closed, public vocabulary only: the tool name, the component name for a component lookup (e.g. `Button`, `Field.Address`), the documentation path for a doc read or list, and the `@dnb/eufemia` version you have installed. No machine identifier, IP address, user or session id, and no free text are ever sent — in particular a `docs_search` query is never collected (a search is recorded as a tool count only).
