...
test.describe('Page Lists', () => {
    ...
    test('should have same amount of components', async ({ page }) => {
      const listLength = await page
        .locator(
          // make exception with:
          // - "infinity-scroller"
          '#portal-sidebar-menu ul li:has(> a[href*="/uilib/components"]) ul li:is(.l-3:has(> a[href*="/components"]):has(> a:not([href*="/fragments"])), .l-4:has(a[href*="/infinity"]))',
        )
        .count()
  ...
  test.describe('of extensions', () => {
    ...
    test('should have same amount of extensions', async ({ page }) => {
      const listLength = await page
        .locator(
          '#portal-sidebar-menu ul li:has(> a[href*="/uilib/extensions"]) ul li.l-3:has(> a[href*="/uilib/extensions/"])',
        )
        .count()
  ...
  test.describe('of elements', () => {
    ...
    test('should have same amount of elements', async ({ page }) => {
      const listLength = await page
        .locator(
          '#portal-sidebar-menu ul li.l-2:has(> a[href*="/uilib/elements"]) ul li:has(> a[href*="/uilib/elements"])',
        )
        .count()
      await expect(
        page.locator(
