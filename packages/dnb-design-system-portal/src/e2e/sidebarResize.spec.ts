...
test.describe('Sidebar resize', () => {
  ...
  test('should resize the sidebar and reset the width on reload', async ({
    ...
    const sidebarLayout = await page.evaluate(() => {
      ...
      const resizeHandle = document.querySelector(
        '[aria-label="Endre størrelse på sidemeny"]'
      )
  ...
  test('should match the SidebarMenu geometry specification', async ({
    ...
    const menu = page.locator(
      '[data-visual-test="sidebar-menu-declarative"] .dnb-sidebar-menu'
    )
    const rootList = menu.locator(':scope > .dnb-sidebar-menu__list')
    const products = menu.locator(
      '[data-sidebar-menu-id="products"] > .dnb-sidebar-menu__item__action'
    )
    const accounts = menu.locator(
      '[data-sidebar-menu-id="accounts"] > .dnb-sidebar-menu__item__action'
    )
    const icon = accounts.locator('.dnb-sidebar-menu__item__icon')
    const iconLeaf = icon.locator('.dnb-icon')
    const expandIcon = products.locator(
      '.dnb-sidebar-menu__accordion__expand-icon'
    )
    const expandIconLeaf = expandIcon.locator('.dnb-icon')
    const header = menu.locator('.dnb-sidebar-menu__header').first()

    await expect(products).toHaveCSS('min-height', '44px')
    await expect(accounts).toHaveCSS('min-height', '44px')
    await expect(rootList).toHaveCSS('gap', '16px')
    await expect(menu.locator('#products-content')).toHaveCSS('gap', '8px')
    await expect(icon).toHaveCSS('width', '24px')
    await expect(icon).toHaveCSS('height', '24px')
    await expect(iconLeaf).toHaveCSS('width', '16px')
    await expect(iconLeaf).toHaveCSS('height', '16px')
    await expect(expandIcon).toHaveCSS('width', '24px')
    await expect(expandIcon).toHaveCSS('height', '24px')
    await expect(expandIconLeaf).toHaveCSS('width', '16px')
    await expect(expandIconLeaf).toHaveCSS('height', '16px')
    await expect(header).toHaveCSS('color', 'rgb(115, 115, 115)')
  })
  ...
  test('should wrap mobile menu content without shifting the logo', async ({
    ...
    const initialTriggerY = await trigger.evaluate(
    ...
    await expect(page.locator('header.sticky-menu')).toHaveAttribute(
      'data-scrolled',
      'true'
    )
    expect(await getVerticalCenterOffset(trigger, headerLogo)).toBe(2)
    expect(
