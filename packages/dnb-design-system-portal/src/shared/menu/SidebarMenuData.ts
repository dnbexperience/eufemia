...
export function insertPlatformToggles(
  items: PortalSidebarMenuItemData[],
  ...
  const platforms = Array.from(
    new Set(items.map(({ platform }) => platform).filter(Boolean))
  )
  if (!platforms.length) {
    return items
  }

  const firstPlatformItemIndex = items.findIndex(
    ({ platform }) => platform
  )
  const firstPlatformItem = items[firstPlatformItemIndex]
  const platformSelector = {
    id: 'portal-platforms',
    type: 'custom' as const,
    dividerBefore: firstPlatformItem.dividerBefore,
    content: createElement(
      'div',
      {
        className: 'portal-sidebar-menu__platforms',
        role: 'group',
        'aria-label': 'Platform',
      },
  ...
  let platformItemIndex = 0

  return items.flatMap((item) => {
    const isFirstPlatformItem = item === firstPlatformItem
    if (item.platform && item.platform !== activePlatform) {
      return isFirstPlatformItem ? [platformSelector] : []
    }
    const visibleItem =
      item.platform && animateItems
        ? {
            ...item,
            className: [
              item.className,
              'portal-sidebar-menu__platform-item',
            ]
              .filter(Boolean)
              .join(' '),
            style: {
              ...item.style,
              animationDelay: `${platformItemIndex++ * 50}ms`,
            },
          }
        : item

    if (!isFirstPlatformItem) {
      return [visibleItem]
    }

    return [platformSelector, { ...visibleItem, dividerBefore: false }]
  })
}
  ...
  return {
    ...
    isActive:
      ...
      config.groups?.some(({ includePageAs }) => includePageAs)
        ...
        : item.isActive,
    isMenuLink:
      config.includePageAs || config.groups?.length
        ? false
        : item.isMenuLink,
        ...
        ? [
            {
              ...
              isMenuLink: true,
              sidebarMenu: undefined,
              subheadings: pageOwnsChildren ? children : undefined,
              _sidebarMenuOrder: config.pageOrder ?? 0,
            },
          ]
        : []),
      ...(pageOwnsChildren ? [] : children),
    ],
...
function sortConfiguredSidebarItems(item: NavItem) {
  item.subheadings?.sort(
    (a, b) =>
      (a._sidebarMenuOrder ?? Number.MAX_SAFE_INTEGER) -
      (b._sidebarMenuOrder ?? Number.MAX_SAFE_INTEGER)
  )
  item.subheadings?.forEach(sortConfiguredSidebarItems)
}
