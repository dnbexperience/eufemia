...
import { docsDir, resolveImportCandidates } from './shared/helpers'
 ...
 * A partial that is also a route of its own is analyzed both ways.
 *
...
describe('portal page ids', () => {
  ...
  it('every served page renders unique ids', () => {
    ...
    for (const page of mdxPages) {
      // A first tab has no route of its own, so it is only checked as part of
      // the parent page that imports it. Every other page is checked, even
      // when a parent also renders it, because it has to hold up on its own
      // route too. `draft` and `hideInMenu` only hide a page from the menu
      // and lists, so those pages are served like any other and are checked.
      if (isFirstTabPage(page)) {
        continue
      }
