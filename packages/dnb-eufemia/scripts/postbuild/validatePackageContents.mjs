 ...
 *
 * Runs `npm pack --dry-run --json` against the built package and fails if the
 * tarball would contain files that must never ship (tests, stories, env files,
 * npmrc, node_modules, editor/OS junk), is missing a required core file, or
 * looks suspiciously empty. This is a security-focused deny-list plus a
 * core-file/size sanity check — it catches leaked sources, secrets and other
 * unexpected files without the churn of a full committed allowlist.
 *
...
// A published tarball with fewer than this many files is almost certainly broken.
export const MIN_FILE_COUNT = 10
...
/**
 * Resolve the list of files `npm pack` would include for the package in `cwd`.
 */
...
function main() {
  ...
  console.log(
    `Package content validation passed: ${files.length} files; entry points and CSS bundles present, no forbidden entries.`
  )
}
