/**
 * Scripts test
 *
 */

import { loadScss } from '../../../../src/core/jest/jestSetup'
import { getFontBasePath } from '../../../../src/plugins/postcss-font-url-rewrite/config'
import { runFactory } from '../makeMainStyle'
import { isCI } from 'repo-utils'

jest.mock('ora', () => {
  return jest.fn(() => ({
    start: jest.fn(),
    info: jest.fn(),
    warn: jest.fn(),
    succeed: jest.fn(),
    fail: jest.fn(),
  }))
})

if (isCI) {
  jest.setTimeout(50e3)

  describe('makeMainStyle transforms "core" SASS to CSS', () => {
    beforeAll(async () => {
      ;(global as any).core = await runFactory(
        './src/style/dnb-ui-core.scss',
        {
          returnResult: true,
        }
      )
    })

    it('has to have valid core css', () => {
      global.console.error = jest.fn()
      const css = loadScss(null, { data: (global as any).core[0] })
      // @ts-expect-error - strictFunctionTypes
      expect(/^Error/.test(css)).toBe(false)
      expect(global.console.error).toHaveBeenCalled()
    })

    it('has to have correct core path to fonts', () => {
      expect((global as any).core[0]).not.toContain('/fonts/dnb/')
    })
  })

  describe('makeMainStyle transforms "components" SASS to CSS', () => {
    beforeAll(async () => {
      ;(global as any).components = await runFactory(
        './src/style/themes/ui/ui-theme-components.scss',
        {
          returnResult: true,
        }
      )
      ;(global as any).files = await runFactory(
        './src/style/themes/ui/ui-theme-components.scss',
        {
          returnFiles: true,
        }
      )
    })

    it('has to have valid components css', () => {
      const css = loadScss(null, { data: (global as any).components[0] })
      // @ts-expect-error - strictFunctionTypes
      expect(/^Error/.test(css)).toBe(false)
    })

    it('has to contain a button selector', () => {
      expect((global as any).components[0]).toMatch(
        new RegExp('.dnb-button\\s?{')
      )
    })

    it('has proper animation names after the cssnano transform', () => {
      expect((global as any).components[0]).not.toMatch(/animation:[a-z] /)
    })

    it('should contain a non minified and a minified content', () => {
      expect((global as any).components[0]).toContain(
        'ATTENTION: This file is auto generated'
      )
      expect((global as any).components[0]).toContain('@charset "UTF-8";')
      expect((global as any).components[0]).not.toContain(';--')
      expect((global as any).components[0]).not.toContain('}:root{--')

      expect((global as any).components[1]).toContain('@charset "UTF-8";')
      expect((global as any).components[1]).toContain(';--')
      expect((global as any).components[1]).toContain('}:root{--')
    })

    it('includes correct files', () => {
      expect((global as any).files).toHaveLength(2)
      expect((global as any).files[0]).toContain(
        '/style/themes/ui/ui-theme-components.css'
      )
      expect((global as any).files[1]).toContain(
        '/style/themes/ui/ui-theme-components.min.css'
      )
    })
  })

  describe('makeMainStyle transforms "elements" SASS to CSS', () => {
    beforeAll(async () => {
      ;(global as any).elements = await runFactory(
        './src/style/dnb-ui-elements.scss',
        {
          returnResult: true,
        }
      )
    })

    it('has to have valid elements css', () => {
      const css = loadScss(null, { data: (global as any).elements[0] })
      // @ts-expect-error - strictFunctionTypes
      expect(/^Error/.test(css)).toBe(false)
    })
  })

  describe('makeMainStyle transforms "theme" SASS to CSS', () => {
    beforeAll(async () => {
      ;(global as any).theme = await runFactory(
        './src/style/themes/ui/ui-theme-basis.scss',
        {
          returnResult: true,
        }
      )
    })

    it('has to have valid theme css', () => {
      const css = loadScss(null, { data: (global as any).theme[0] })
      // @ts-expect-error - strictFunctionTypes
      expect(/^Error/.test(css)).toBe(false)
    })

    it('has to have correct custom properties', () => {
      expect((global as any).theme[0]).toMatch(
        new RegExp('--color-sea-green:\\s?#007272;')
      )
      expect((global as any).theme[0]).toMatch(
        new RegExp('color:\\s?var\\(--color-sea-green\\);')
      )
      expect((global as any).theme[0]).not.toMatch(
        new RegExp('color:\\s?#007272;')
      )
      expect((global as any).theme[0]).not.toContain('fuchsia')
    })

    it('has to have correct core path to fonts', () => {
      expect((global as any).theme[0]).toMatch(
        new RegExp('("|\\()../../../assets/fonts/dnb/')
      )
    })
  })

  describe('makeMainStyle with enableBuildStyleScope', () => {
    // Ensure enableBuildStyleScope returns true
    let originalEnv: any
    beforeAll(() => {
      originalEnv = process.env.NODE_ENV
      process.env.NODE_ENV = 'production'
      jest.resetModules()
    })
    afterAll(() => {
      process.env.NODE_ENV = originalEnv
    })

    // Mock console.log to suppress lines containing '✨'
    const originalConsoleLog = console.log
    beforeAll(() => {
      console.log = (...args) => {
        if (
          args.some((arg) => typeof arg === 'string' && arg.includes('✨'))
        )
          return
        originalConsoleLog(...args)
      }
    })
    afterAll(() => {
      console.log = originalConsoleLog
    })

    // Run the factory
    beforeAll(async () => {
      const { runFactory } = await import('../makeLibStyles')
      ;(global as any).css = await runFactory(
        './src/style/themes/ui/ui-theme-basis.scss',
        {
          returnResult: true,
        }
      )
      ;(global as any).files = await runFactory(
        './src/style/themes/ui/ui-theme-basis.scss',
        {
          returnFiles: true,
        }
      )
    })

    it('should transform CSS to have scoped selectors', async () => {
      expect((global as any).css[0]).toContain('.eufemia-scope--default ')

      const count = (
        (global as any).css[0].match(/\.eufemia-scope--default /g) || []
      ).length
      expect(count).toBeGreaterThan(50)
    })

    it('should contain the DNB Skeleton font URL in the CSS', () => {
      expect((global as any).css[0]).toContain(
        `${getFontBasePath()}dnb/DNB-Regular.woff2`
      )
    })

    it('should generate isolated CSS files when enableBuildStyleScope is true', async () => {
      expect(
        (global as any).files.some((f: any) =>
          f.includes('--isolated.css')
        )
      ).toBe(true)
      expect(
        (global as any).files.some((f: any) =>
          f.includes('--isolated.min.css')
        )
      ).toBe(true)
    })
  })
} else {
  it('skipping local tests', () => {
    expect(true).toBe(true)
  })
}
