...
const transformToJsx = (content, file): PromiseLike<string> => {
  ...
  const iconSize = Object.entries(ICON_SIZES).find(
    ([, { suffix }]) =>
      suffix && filename.endsWith(`${NAME_SEPARATOR}${suffix}`)
  )?.[0]
        ...
        .then(async (res) => {
          ...
          if (iconSize) {
            typed = typed.replace(
              ...
              `export default /* @__PURE__ */ Object.assign(${componentName}, { __iconSize: ${iconSize} as const });`
            )
          }
