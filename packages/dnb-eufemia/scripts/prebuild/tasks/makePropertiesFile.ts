...
const transformFigmaPath = (path: string[]) => {
  ...
  if (replacedCaracters.length > 0) {
    log.warn(
      `> PrePublish: "makePropertiesFile" replaced characters "${replacedCaracters.join(
        '", "'
      )}" in Figma variable "${filteredPath.join(
        '/'
      )}". Should be renamed in Figma.`
    )
  }
...
const makeFigmaSCSS = async (
  ...
  log.info(
    `> PrePublish: "makePropertiesFile" Figma sass file | ${filePath}`
  )
}
