...
function DropdownInstance(props) {
  ...
  const setVisible = () => {
    context.drawerList.setWrapperElement(refWrapper.current).setVisible()
  }

  const setHidden = (...args) => {
    context.drawerList.setHidden(...args)
  }

  const onFocusHandler = () => {
    if (isTrue(props.openOnFocus)) {
      setVisible()
    }
  }
  ...
  const onTriggerKeyDownHandler = (e) => {
    switch (keycode(e)) {
      case 'enter':
      case 'space':
        e.preventDefault()
    ...
    focusTimeoutRef.current = setTimeout(() => {
      try {
        ...
        if (element && typeof element.focus === 'function') {
          ...
          dispatchCustomElementEvent({ props }, 'onCloseFocus', {
            element,
          })
        }
      } catch (e) {
  ...
  const extendedProps = extendPropsWithContextInClassComponent(
    ...
    context.getTranslation(props).Dropdown,
  ...
  let { icon, iconPosition, alignDropdown } = extendedProps
  ...
  validateDOMAttributes(props, triggerParams)
  ...
  return (
    <span ref={ref} {...mainParams}>
      ...
      <span className="dnb-dropdown__inner" ref={refWrapper}>
        <AlignmentHelper />
...
DropdownInstance.propTypes = Dropdown.propTypes
