...
function ListFormat(
  ...
  // Only the text variant is worth memoizing – it builds an Intl.ListFormat.
  // The list variants render directly so they pick up every prop change.
  const text = useMemo(
    () =>
      variant === 'text' ? listFormat(list, { locale, format }) : null,
