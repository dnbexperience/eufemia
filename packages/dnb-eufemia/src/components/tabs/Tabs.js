...
export default class Tabs extends React.PureComponent {
  ...
  init = () => {
    if (this._isMounted) {
      ...
      this.scrollToTab({ type: 'focus', behavior: 'auto' })
      this.scrollToTab({ type: 'selected', behavior: 'auto' })
  ...
  saveLastPosition(pos = this._tablistRef.current.scrollLeft) {
    if (typeof window !== 'undefined') {
      try {
        window.localStorage.setItem(`tabs-pos-${this._id}`, pos) // gets removed right afterwards
      } catch (e) {
        warn(e)
      }
    }
    ...
    const delay = () => {
      try {
        if (this.state.hasScrollbar && this._tablistRef.current) {
          ...
          // Used to add the extra padding when we go from 1 to 2 tab so the nav button
          // Now we use CSS only to apply this needed extra padding
          // if (!isFirst && parseFloat(style.paddingLeft) < 16) {
