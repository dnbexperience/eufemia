...
export type ToasterMessage = {
  ...
  /**
   * Visual variant of the message. Defaults to `neutral`.
   */
  ...
  /**
   * Priority of the announcement to assistive technology. `low` waits for idle, `high` interrupts immediately. Defaults to `low`.
   */
  ...
  /**
   * Privacy level of the message content. When set to `sensitive`, the notification center shows `privacyFallbackText` instead of the original text, and hides actions. The toast stack still shows the full content. Defaults to `normal`.
   */
  privacy?: ToasterPrivacy

  /**
   * Text shown in place of the original text in the notification center for sensitive messages.
   */
...
export type ToasterProps = {
  ...
  /**
   * Content to render inside the Toaster. When provided, the Toaster acts as both a provider and a renderer.
   */
  children?: ReactNode
} & SpacingProps
