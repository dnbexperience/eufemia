...
describe('WordDefinition', () => {
  it('renders trigger with closed state by default', () => {
    render(
      <WordDefinition content={definition}>{term}</WordDefinition>
    )
  ...
  it('wraps the header term with guillemets (« ») internally', async () => {
    render(
      <WordDefinition content={definition}>{term}</WordDefinition>
    )
    const trigger = document.querySelector(
    ...
    await waitFor(() => {
      const titleWrapper = document.querySelector(
        '.dnb-word-definition__title'
      )
      const headerTerm = document.querySelector(
        '.dnb-word-definition__title strong'
      )
      expect(headerTerm.textContent).toBe(term)
      expect(titleWrapper.textContent).toBe(`«${term}»`)
    })
  })
