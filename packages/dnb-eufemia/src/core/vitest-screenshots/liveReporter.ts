 ...
 * end) and `SummaryOnlyReporter`, which preserves Vitest's final
 * summary without re-printing every completed test from each file.
 */
...
export default class LiveReporter implements Reporter {
  ...
  onTestCaseResult(testCase: TestCase) {
    ...
    if (result.state === 'passed') {
      ...
      const retryInfo =
        retryCount > 0
          ? ` ${color('yellow', `(retry #${retryCount})`)}`
          : ''
      write(
        `${color('green', '✓')} ${counter} ${testCase.fullName}${retryInfo}` +
          (duration !== undefined
      ...
      const messages = result.errors
        ...
        .filter(Boolean)
      write(
        `${color('red', '✗ FAILED')} ${counter} ${testCase.fullName}${color('red', retryInfo)}`
      )
      messages.forEach((m) => write(color('red', `  ${m}`)))
      this.failed.push(testCase.fullName)
  ...
  onTestRunEnd(
    ...
    if (this.failed.length > 0) {
      write('')
      write(
        color('red', `Screenshot tests failed (${this.failed.length}):`)
      )
      this.failed.forEach((name) => {
        write(color('red', `  ✗ ${name}`))
      })
      write('')
    }
  }
