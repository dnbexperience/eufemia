...
export const ProviderProperties: PropertiesTableProps = {
  ...
  ajvInstance: {
    doc: 'REQUIRED when using JSON Schema validation. Install `ajv` and `ajv-errors` as dependencies, then use `makeAjvInstance` from `@dnb/eufemia/extensions/forms/utils/ajv` and pass it as `ajvInstance={makeAjvInstance()}`. This ensures your bundle only includes AJV when you actually need it. More info in the [Schema validation](/uilib/extensions/forms/Form/schema-validation/#custom-ajv-instance-and-keywords) section.',
    type: '"ajv"',
    status: 'optional',
  },
  transformIn: {
