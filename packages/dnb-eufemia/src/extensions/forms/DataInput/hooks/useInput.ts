...
export default function useInput<Props extends InputProps<any>>(props: Props): Omit<Props, keyof ReturnPropOverrides> & ReturnPropOverrides  {
  const {
    id = makeUniqueId(),
    path,
    emptyValue,
  ...
  const [showError, setShowError] = useState<boolean>(Boolean(validateInitially || errorProp));
  const schemaValidator = useMemo(
    () => ((schema && Object.keys(schema).length > 0) ? ajv.compile(schema) : undefined),
    [schema]
  );

  const setErrorAndUpdateDataContext = useCallback((error: FormError | undefined) => {
    setError(error);
  ...
  const validateValue = useCallback((valueToValidate): FormError | undefined => {
    ...
    if (valueToValidate === emptyValue && required) {
      const error = new FormError('The value is required', { validationRule: 'required' }); // TO DO: Lib-specific translations
 ...
 useEffect(() => {
  ...
  const onFocus = useCallback(({ onFocusValue }) => {
    onFocusProp?.(onFocusValue ?? value);
  }, [value, onFocusProp]);

  const onBlur = useCallback(({ onBlurValue }) => {
    ...
    setShowError(true);
  }, [value, onBlurProp, validateUnchanged, onBlurValidatorProp, setErrorAndUpdateDataContext]);
  ...
  }, [path, onChangeProp, validateValue, dataContextHandlePathChange, fromInput]);
  ...
  const exportError = errorProp ?? (
    (error instanceof FormError && typeof error.validationRule === 'string' && errorMessages?.[error.validationRule] !== undefined)
