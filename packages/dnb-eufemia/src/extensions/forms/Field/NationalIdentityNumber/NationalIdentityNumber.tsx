...
import { validateDnr, validateFnr, validateDnrAndFnr } from './validators'
...
function NationalIdentityNumber(props: FieldNationalIdentityNumberProps) {
  ...
  const fnrValidator = useCallback(
    (value: string) => validateFnr(value, { errorFnr, errorFnrLength }),
    [errorFnr, errorFnrLength]
  )

  const dnrValidator = useCallback(
    (value: string) => validateDnr(value, { errorDnr, errorDnrLength }),
    [errorDnr, errorDnrLength]
  )

  const dnrAndFnrValidator = useCallback(
    (value: string) =>
      validateDnrAndFnr(value, {
        errorFnr,
        errorFnrLength,
        errorDnr,
        errorDnrLength,
