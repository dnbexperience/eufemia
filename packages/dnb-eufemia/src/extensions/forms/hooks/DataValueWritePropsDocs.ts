...
export const DataValueWritePropsProperties: PropertiesTableProps = {
  ...
  labelSuffix: {
    doc: 'Will append additional text to the label, like "(optional)", displayed in a secondary text color. When using `inheritLabel`, the suffix will not be inherited.',
...
export const DataValueWritePropsEvents: PropertiesTableProps = {
  onChange: {
    doc: "Will be called on value changes made by the user, with the new value as argument. When an `async` function is used, the corresponding [FieldBlock](/uilib/extensions/forms/create-component/FieldBlock/) will show an indicator for the field while it is pending. You can return `{ success: 'saved' } as const` to show a success symbol, or an error or an object with these keys `{ info: 'Info message', warning: 'Warning message', error: Error('My error') } as const`. The second parameter is an object that e.g. contains `properties` (all given `Field.*` properties). If an `async` function does not settle within `asyncSubmitTimeout` (30 seconds by default), the field clears its pending state, so a Promise that never settles does not leave the field stuck. A submit waiting for that field is canceled and must be retried.",
    type: '(value) => void',
    status: 'optional',
  },
  onFocus: {
