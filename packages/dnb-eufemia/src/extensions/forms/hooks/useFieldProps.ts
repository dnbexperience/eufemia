...
export default function useFieldProps<Value, EmptyValue, Props>(
    ...
    if (schema || dataContext?.schema) {
      ...
      if (paths.length > 0) {
        ...
        if (paths.length > 1) {
          ...
          const requiredSchemaList = (
            schemaPart as Record<string, unknown>
          )?.['required']
  ...
  const fieldBlockProps = {
    ...
    labelHeight:
      typeof (props as Record<string, unknown>)['size'] === 'string'
        ? ((props as Record<string, unknown>)['size'] as string)
        : undefined, // component/field size
  }
