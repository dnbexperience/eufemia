...
export function formatNumber(value: number, options?: FormatNumberOptions): string {
  ...
  const withMagnitude = magnitude !== undefined
    ? value / Math.pow(10, magnitude)
    : value;

  const withDecimals = fixedDecimals !== undefined
    ? withMagnitude.toFixed(fixedDecimals)
    : (decimals
      ? (Math.round(withMagnitude * Math.pow(10, decimals)) / Math.pow(10, decimals)).toString()
      : withMagnitude.toString()
    );
  
  const withDecimalSymbol = decimalSymbol !== undefined
    ? withDecimals.replace('.', decimalSymbol)
    : withDecimals;
  
  const withThousandSeparator = thousandSeparator !== undefined
    ? withDecimalSymbol.replace(/\B(?=(\d{3})+(?!\d))/g, thousandSeparator)
    : withDecimalSymbol;
  
  const withPrefix = prefix !== undefined
    ? prefix + withThousandSeparator
    : withThousandSeparator;
  
  const withSuffix = suffix !== undefined
    ? withPrefix + suffix
    : withPrefix;
...
export function parseFormattedNumber(formattedValue: string, options?: FormatNumberOptions): number {
  const {
    thousandSeparator,
    decimalSymbol = ',', // Norwegian default format
    magnitude,
  ...
  const parsedDecimalSymbol = decimalSymbol !== undefined
    ? formattedValue.replace(decimalSymbol, '.')
    : formattedValue;
  
  const parsedThousandSeparator = thousandSeparator !== undefined
    ? parsedDecimalSymbol.split(thousandSeparator).join('')
    : parsedDecimalSymbol;
  
  const parsedPrefix = (prefix !== undefined && parsedThousandSeparator.substring(0, prefix.length) === prefix)
    ? parsedThousandSeparator.substring(prefix.length)
    : parsedThousandSeparator;
  
  const parsedSuffix = (suffix !== undefined && parsedPrefix.slice(suffix.length * -1) === suffix)
    ? parsedPrefix.substring(0, parsedPrefix.length - suffix.length)
    : parsedPrefix;
  
  const parsedNumber = Number(parsedSuffix);
  
  const parsedMagnitude = magnitude !== undefined
