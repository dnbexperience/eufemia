...
> & {
  ...
  /** Width reserved while the inline sidebar is compact. Default: 4rem. */
...
export default function SidebarMenuResponsiveAside({
  ...
  useIsomorphicLayoutEffect(() => {
    if (!inlineRef.current || !onWidthChange) {
      return undefined
    }

    const reportWidth = () => {
      onWidthChange(inlineRef.current?.getBoundingClientRect().width ?? 0)
    }
    reportWidth()
