 ...
 */
export type DynamicElement<E = HTMLElement, P = React.DetailedHTMLProps<React.HTMLAttributes<E>, E>> = React.ElementType<P> | string;
export type DynamicElementParams<T = Record<string, unknown>> = T;
export type PropertiesTableProps = Record<string, {
    type: string | string[];
    defaultValue?: string;
    doc: string;
