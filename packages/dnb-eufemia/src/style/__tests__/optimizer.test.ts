import fs, { mkdtempSync, mkdirSync, writeFileSync, rmSync } from 'fs'
import os from 'os'
...
describe('createSafelist', () => {
  ...
  it('loads the generated build manifest by default', () => {
    ...
    try {
      ...
      expect(result.components).toContain('button')
      expect(result.prefixes).toContain('dnb-button')
    } finally {
      readFileSync.mockRestore()
    }
  })

  it('throws an actionable error when the generated manifest is missing', () => {
    const existsSync = vi
      .spyOn(fs, 'existsSync')
      .mockImplementation((file) =>
        String(file).endsWith('package.json')
          ? true
          : !String(file).endsWith('style-manifest.json')
      )

    try {
      expect(() => createSafelist({ components: ['button'] })).toThrow(
        /run the @dnb\/eufemia package preparation/i
      )
    } finally {
  ...
  it('rejects a custom manifest with an unsupported schema version', () => {
    expect(() =>
      createSafelist({
        manifest: { ...manifest, version: 2 },
        components: ['button'],
      })
    ).toThrow(/expected version 1/i)
  })
  ...
  it('rejects a manifest without an entries object', () => {
    expect(() =>
      createSafelist({
        manifest: { version: 1 } as StyleManifest,
        components: ['button'],
      })
    ).toThrow(/expected an entries object/i)
  })
...
describe('detectUsedComponents', () => {
  ...
  it('detects public element aliases from main, elements, and deep imports', () => {
    write(
      'a.tsx',
      `import { Link, Td } from '@dnb/eufemia'\n` +
        `import { Th } from '@dnb/eufemia/elements'\n` +
        `import Tr from '@dnb/eufemia/elements/Tr'`
    )
  ...
  it('detects direct re-exports and ignores type-only re-exports', () => {
    write(
      'a.ts',
      [
        `export { Button, Link as TextLink } from '@dnb/eufemia'`,
        `export { default as BodyRow } from '@dnb/eufemia/elements/Tr'`,
        `export type { ButtonProps } from '@dnb/eufemia'`,
      ].join('\n')
    )
  ...
  it('detects Eufemia Forms from a source (/src) forms-subpath import', () => {
    write(
      'a.tsx',
      `import { Field, Form } from '@dnb/eufemia/${sourcePackageSegment}/extensions/forms'`
    )
  ...
  it('resolves RegisteredField and RegisteredValue aliases', () => {
    write(
      'a.tsx',
      [
        `import { RegisteredField, RegisteredValue as V } from '@dnb/eufemia/extensions/forms'`,
        `const App = () => (<>`,
        `  <RegisteredField.Slider path="/range" />`,
        `  <V.Upload path="/files" />`,
        `</>)`,
      ].join('\n')
    )
  ...
  it('resolves members through a Forms namespace import', () => {
    write(
      'a.tsx',
      [
        `import * as Forms from '@dnb/eufemia/extensions/forms'`,
        `const App = () => (<>`,
        `  <Forms.Field.Date path="/date" />`,
        `  <Forms.RegisteredValue.Upload path="/files" />`,
        `</>)`,
      ].join('\n')
    )
  ...
  it('resolves a Forms namespace member assigned to a local binding', () => {
    write(
      'a.tsx',
      [
        `import * as Forms from '@dnb/eufemia/extensions/forms'`,
  ...
  it('keeps all member dependencies for unresolved Forms namespace indirection', () => {
    write(
      'a.tsx',
      [
        `import * as Forms from '@dnb/eufemia/extensions/forms'`,
        `const IndirectForms = Forms`,
        `const DynamicField = Forms[namespace][member]`,
      ].join('\n')
    )
  ...
  it('resolves members from Field and Value member barrels', () => {
    write(
      'a.tsx',
      [
        `import { Upload as UploadField } from '@dnb/eufemia/extensions/forms/Field'`,
        `import { Upload } from '@dnb/eufemia/extensions/forms/Value'`,
        `const App = () => <><UploadField /><Upload /></>`,
      ].join('\n')
    )
  ...
  it('resolves the compatibility default namespace from Forms', () => {
    write(
      'a.tsx',
      [
        `import Forms from '@dnb/eufemia/extensions/forms/Forms'`,
        `const App = () => <Forms.Slider path="/range" />`,
      ].join('\n')
    )
  ...
  it('resolves straightforward Forms member destructuring and aliases', () => {
    write(
      'a.tsx',
      [
        `import { Field } from '@dnb/eufemia/extensions/forms'`,
        `const { Upload, Name: NameField } = Field`,
        `const App = () => <><Upload /><NameField /></>`,
      ].join('\n')
    )
  ...
  it('keeps all member-specific dependencies for unresolved Forms member usage', () => {
    write(
      'a.tsx',
      [
        `import { Field } from '@dnb/eufemia/extensions/forms'`,
        `const DynamicField = Field[fieldName]`,
        `const App = () => <DynamicField />`,
      ].join('\n')
    )
  ...
  it('resolves member dependencies from Forms deep imports', () => {
    write(
      'a.tsx',
      `import Upload from '@dnb/eufemia/extensions/forms/Field/Upload'`
    )
    write(
      'b.tsx',
      `import ValueUpload from '@dnb/eufemia/${sourcePackageSegment}/extensions/forms/Value/Upload/index.ts'`
    )
  ...
  it('derives Forms namespaces from the manifest', () => {
    const customManifest: StyleManifest = {
      ...manifest,
      formsFieldDependencies: {
    ...
    write(
      ...
      [
        `import * as Forms from '@dnb/eufemia/extensions/forms'`,
        `const App = () => <Forms.RegisteredDisplay.Upload />`,
      ].join('\n')
    )

    const result = detectUsedComponents({
  ...
  it('detects deep imports from the source (/src) path', () => {
    write(
      'a.tsx',
      `import Button from '@dnb/eufemia/${sourcePackageSegment}/components/Button'`
    )
