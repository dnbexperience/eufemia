...
async function generateProptypes(
  ...
  console.log("body", proptypes.body)
...
async function run(argv: HandlerArgv) {
  ...
  if (pattern.length > 0) {
    console.log(
      `Only considering declaration files matching ${filePattern}`
    )
  }
  ...
  const files = _.flatten(allFiles)
    ...
    .filter((filePath) => {
      ...
      // Our folder names are lowercase but components in them are uppercase
      // Convert files to lowercase on comparing if they match folder name
      const fileName = path.basename(filePath, '.d.ts').toLowerCase()
    ...
    builder: (command) => {
      return command
        ...
        .option('pattern', {
          default: '',
          describe:
            'Only considers declaration files matching this pattern.',
          type: 'string'
        })
    },
    handler: run
