...
The CloudFront origin is added to the dashboard API's CORS automatically (the distribution domain is always included in the API's CORS configuration), so no second deploy is needed for cross-origin `/data` calls; extra origins such as a local-dev URL can be added via the optional `dashboard_origins` Terraform variable. After the first deploy, add the CloudFront URL as a redirect URI on the app registration.
