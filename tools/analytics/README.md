...
The dashboard API's CORS allows the dashboard's canonical origin (`DASHBOARD_PUBLIC_URL`, the custom domain) plus any extra `DASHBOARD_ORIGINS` such as a local-dev URL. The raw `*.cloudfront.net` origin is intentionally not allowed, because direct-CloudFront access bypasses Akamai and the WAF. After the first deploy, add the dashboard URL as a redirect URI on the app registration.
