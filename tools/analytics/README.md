...
- **S3 bucket** (versioned, SSE-S3, public access blocked) holding portal-view records (`portal-views/`), MCP usage (`mcp-usage/`, `mcp-usage-daily/`), component usage (`component-usage/`), the dashboard snapshot (`snapshots/dashboard.json`), and Athena output (`athena-results/`, expired after 7 days).
- **Glue database + tables** with JSON SerDe and partition projection on `dt` (`portal_views`, `mcp_usage`, `mcp_usage_daily`, `component_usage`).
