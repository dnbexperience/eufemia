...
> **Note:** the snapshot generator refreshes the durable `portal_views_daily` rollup each run for retention, but the dashboard does not read it yet — it still shows the recent raw page-view rows. Surfacing the retained history (and its dimensions) on the dashboard is a follow-up; the rollup exists now so the history is preserved before the raw rows begin to expire. On its first run (empty rollup) the generator backfills the full raw history in one pass, so page-views recorded before this rollup existed are captured before the retention rule can delete them; every later run only recomputes the recent tail.
>
