locals {
  service_name = "eufemia-${var.environment}-assets"
...
resource "aws_s3_bucket" "assets" {
  bucket = "${local.service_name}-${data.aws_caller_identity.current.account_id}"
  tags   = local.tags
}
...
resource "aws_cloudfront_function" "edge_auth" {
  name    = "${local.service_name}-edge-auth"
  runtime = "cloudfront-js-2.0"
  comment = "Allow the Akamai edge to reach the Eufemia asset origin"
  publish = true
  code = templatefile("${path.module}/functions/edge-auth.js.tftpl", {
...
resource "aws_cloudfront_distribution" "assets" {
  ...
  comment         = "Eufemia public asset origin behind Akamai"
  tags            = local.tags
