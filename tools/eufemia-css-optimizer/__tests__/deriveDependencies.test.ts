...
describe('deriveClassDependencies', () => {
  const prefixIndex = buildPrefixIndex(['button', 'icon', 'tooltip'])

  it('ignores class tokens in comments while preserving string literals', () => {
    const dependencies = deriveClassDependencies(
      [
        '// example: <Button className="dnb-tooltip" />',
        '/* className="dnb-tooltip" */',
        'const iconClass = "dnb-icon"',
      ].join('\n'),
      'button',
      prefixIndex
    )

    expect(dependencies).toEqual(['icon'])
  })
})
...
describe('deriveDirectDependencies', () => {
  ...
  it('detects dependencies in nested theme styles', () => {
    const themesDir = path.join(styleDir, 'themes', 'ui')
    fs.mkdirSync(themesDir, { recursive: true })
    fs.writeFileSync(
      path.join(themesDir, 'dnb-button-theme-ui.scss'),
      '.dnb-tooltip { color: green; }'
    )
