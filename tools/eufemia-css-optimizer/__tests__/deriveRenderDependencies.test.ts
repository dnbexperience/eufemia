...
describe('deriveRenderDependencies', () => {
  ...
  beforeAll(() => {
    ...
    make(
      ...
      [
        ...
        "import type { DrawerShow } from '../drawer/Drawer'",
  ...
  const index = () =>
    buildComponentDirIndex([
      ...
      { name: 'space', dir: path.join(root, 'components', 'space') },
      { name: 'drawer', dir: path.join(root, 'components', 'drawer') },
  ...
  it('treats every runtime import from a styled sibling as a dependency', () => {
    const deps = deriveRenderDependencies({
      componentDir: buttonDir,
      name: 'button',
      componentDirIndex: index(),
    ...
    expect(deps).toContain('anchor')
    expect(deps).toContain('space')
    expect(deps).toContain('skeleton')
  })
  ...
  it('ignores type-only imports', () => {
    ...
    expect(deps).not.toContain('drawer')
  })
...
describe('deriveRenderDependencies via the main group barrel', () => {
  let root: string
  let buttonDir: string
  let componentsDir: string

  beforeAll(() => {
    root = fs.mkdtempSync(path.join(os.tmpdir(), 'render-main-barrel-'))
    componentsDir = path.join(root, 'components')
    buttonDir = path.join(componentsDir, 'button')

    fs.mkdirSync(buttonDir, { recursive: true })
    fs.mkdirSync(path.join(componentsDir, 'tooltip'), { recursive: true })
    fs.writeFileSync(
      path.join(buttonDir, 'Button.tsx'),
      [
        "import { Tooltip } from '..'",
        'export const Button = () => <Tooltip />',
      ].join('\n')
    )
  })
  ...
  it('resolves runtime imports from a group barrel outside Forms', () => {
    const deps = deriveRenderDependencies({
      componentDir: buttonDir,
      name: 'button',
      componentDirIndex: buildComponentDirIndex([
        { name: 'button', dir: buttonDir },
        { name: 'tooltip', dir: path.join(componentsDir, 'tooltip') },
      ]),
      barrelDirs: [componentsDir],
      barrelExports: new Map([['Tooltip', 'tooltip']]),
    })
