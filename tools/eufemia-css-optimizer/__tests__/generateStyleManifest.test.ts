...
describe('generateStyleManifest', () => {
  ...
  it('is deterministic', () => {
    expect(generateStyleManifest({ sourceRoot })).toEqual(manifest)
    expect(
      JSON.stringify(generateStyleManifest({ sourceRoot }), null, 2)
  ...
  it('does not pull Forms into country selection components through shared country data', () => {
    expect(manifest.entries['country-flag'].dependencies).not.toContain(
      'forms'
    )
    expect(manifest.entries.dropdown.dependencies).not.toContain('forms')
    expect(manifest.entries.autocomplete.dependencies).not.toContain(
      'forms'
    )
  })
  ...
  it('captures runtime helper imports from space and skeleton', () => {
    expect(manifest.entries.tabs.dependencies).toContain('space')
    expect(manifest.entries.tabs.dependencies).toContain('skeleton')
  })

  it('captures raw class ownership from shippable source', () => {
    expect(
      manifest.formsFieldDependencies['Field.MultiSelection']
    ).toContain('dropdown')
    expect(manifest.entries.textarea.dependencies).toContain('input')
    expect(manifest.entries.pagination.dependencies).toContain('table')
  })

  it('resolves main-barrel imports for regular components', () => {
    expect(manifest.entries.filter.dependencies).toContain('button')
  })
  ...
  describe('foundation prefixes', () => {
    ...
    it('excludes ordinary standalone entries and structural globals', () => {
      // Most standalone entries are not duplicated into the foundation. `space`
      // and `skeleton` are intentional exceptions because every Element can emit
      // their classes through spacing/skeleton props.
      for (const prefix of manifest.foundation) {
        if (prefix === 'dnb-space' || prefix === 'dnb-skeleton') {
          continue
        }
      ...
      expect(manifest.foundation).toContain('dnb-space')
      expect(manifest.foundation).toContain('dnb-skeleton')
  ...
  describe('forms field dependencies', () => {
    ...
    it('moves field-specific components out of the base entry', () => {
      ...
      // Other member-only blocks stay out of the base. Input is now retained by
      // the base because shared FormLabel source emits an input class directly.
      expect(forms.dependencies).toContain('input')
    ...
    it('attributes a field its own component dependencies', () => {
      // Input is already retained by the shared forms base.
      expect(fields['Field.String']).toContain('input-masked')
    ...
    it('lets composite fields inherit their sub-field dependencies', () => {
      // Field.Email renders Field.String internally, so it inherits its extras.
      expect(fields['Field.Email']).toContain('input-masked')
