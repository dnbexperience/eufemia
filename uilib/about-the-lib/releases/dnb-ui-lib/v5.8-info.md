...
version: 11.8.1
generatedAt: 2026-06-29T11:30:02.381Z
...
- added `min_date` and `max_date` properties, with an event return of `is_valid_start_date` and `is_valid_end_date`
- got a new property `show_reset_button` to show a _Reset_ button inside the DatePicker footer / toolbar.
...
- have `dnb-core-style` class set by default to the Modal content - prevent this by using the `prevent_core_style` property.

[Anchor / Text link](/uilib/components/anchor)

- `target="_blank"` with `href="http*"` will get an icon added, showing that the link is a "link out" of the current context. You can prevent the link from appearing by using an HTML class: `dnb-anchor--no-icon`.
...
- added `border` property to show a rounded border on icons.
