...
1. If you used `style_type`, like `<H1 style_type="small">` before, instead use `<H1 size="x-large">` and the same applies to `<P style_type="small">`, so use the `modifier` property here `<P modifier="small">`
...
We hope that this change promotes [better accessibility](/uilib/usage/accessibility/checklist).
...
- Icons inside of [Input](/uilib/components/input/demos#input-with-icon) fields.
