...
metadata: https://eufemia.dnb.no/uilib/about-the-lib/releases/dnb-ui-lib/v7.2-info/metadata.json
---
...
- [DatePicker](/uilib/components/date-picker) got an internal overhauling, new styles and an `on_days_render` render property as well as `on_type` event because of the new UX. Now it is possible to invalidate a part of a in the input.
...
- [Modal & Drawer](/uilib/components/modal) new UX styles and properties like `spacing` and an helper wrapper called `Modal.Inner`.
