...
version: 11.7.0
generatedAt: 2026-06-22T08:28:01.335Z
...
If rendered output depends on one form data path inside `Form.Handler`, use [Form.useDataValue](/uilib/extensions/forms/Form/useDataValue/).

If you need the imperative helpers without subscribing to form data changes, use `Form.useDataWithoutSubscription`.

For custom field or value components that render a form value, prefer [useFieldProps](/uilib/extensions/forms/create-component/useFieldProps/) or [useValueProps](/uilib/extensions/forms/create-component/useValueProps/). They keep the component reactive with path-scoped form updates.

## Relevant links

- [Source code](https://github.com/dnbexperience/eufemia/tree/main/packages/dnb-eufemia/src/extensions/forms/Form/DataContext/useData.ts)
- [Source code for useDataWithoutSubscription](https://github.com/dnbexperience/eufemia/tree/main/packages/dnb-eufemia/src/extensions/forms/Form/DataContext/useDataWithoutSubscription.ts)
...
Use [Form.useDataValue](/uilib/extensions/forms/Form/useDataValue/) when rendered output depends on a single form value inside `Form.Handler`. It subscribes only to that path.

### Without subscription

`Form.useData` subscribes the hook consumer to form data changes. This is useful when rendered output depends on the data.

If rendered output depends on one value, use [Form.useDataValue](/uilib/extensions/forms/Form/useDataValue/) to subscribe only to that path.

For custom fields and other internal integrations that only need imperative access to form data, use `Form.useDataWithoutSubscription`. It accepts the same optional `id` and `initialData` arguments as `Form.useData`, but does not subscribe to form data updates.

It returns `getData`, `getValue`, `update`, `remove`, `set`, `filterData` and `reduceToVisibleFields`. It does not return `data`, because `data` would be a stale render snapshot after form changes. Use `getData` or `getValue` when you need the latest value without subscribing.

`Form.useDataWithoutSubscription` is not a replacement for [useFieldProps](/uilib/extensions/forms/create-component/useFieldProps/) or [useValueProps](/uilib/extensions/forms/create-component/useValueProps/) inside custom fields that render form data. Use those hooks for the rendered field value, and use `Form.useDataWithoutSubscription` only for additional imperative reads or writes in callbacks, effects or internal handlers.
...
If you need to render fields, values, or other `Form.*` components in another part of the component tree, use [Form.Outlet](/uilib/extensions/forms/Form/Outlet/) with the same form handler id (`formHandlerId`).
