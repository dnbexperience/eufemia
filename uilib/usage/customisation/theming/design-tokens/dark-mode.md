---
version: 11.2.1
generatedAt: 2026-05-08T08:59:11.627Z
...
```tsx
render(<div style={darkModeGridStyle}>
      {schemes.map(scheme => <Card key={scheme} title={scheme === 'dark' ? 'Dark color scheme' : 'Light color scheme'}>
          <Theme colorScheme={scheme}>
            <Flex.Stack gap="small">
              {darkModeSwatchRows.map(row => <DarkModeCard key={`${scheme}-${row.label}`} scheme={scheme} row={row} />)}
            </Flex.Stack>
          </Theme>
        </Card>)}
    </div>)
